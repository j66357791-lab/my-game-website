...
const AnimalGameEngine = require('./utils/game-engine');

// 导入动物大冒险路由
const animalGameRouter = require('./routes/animal-game');

// 导入太空挑战游戏相关模块
const SpaceGameBet = require('./models/SpaceGameBet');
const { adjustBossEconomy } = require('./utils/space-game-economy');
const { router: spaceGameRouter, currentGame } = require('./routes/space-game');
...
io.on('connection', (socket) => {
    ...
    socket.on('joinGame', (data) => {
        if (data && data.userId) {
            ...
            socket.join('animal-game');
            console.log(`🎮 用户 ${data.userId} 加入动物大冒险`);
        }
    });
    
    // 加入太空挑战游戏
    socket.on('join-space-game', (data) => {
        if (data && data.userId) {
            socket.userId = data.userId;
            socket.join('space-game');
            console.log(`🚀 用户 ${data.userId} 加入太空挑战`);
        }
    });
    ...
    socket.on('lifespan-changed', async (data) => {
        try {
            ...
            // 通知家庭所有成员
            io.to(`family_${familyId}`).emit('lifespan-changed', {
                chickenId: chickenId,
...
// 初始化动物大冒险游戏引擎
const gameEngine = new AnimalGameEngine(io);
...
// ==================== 动物大冒险游戏 API ====================
// 使用路由文件 - 删除所有冲突的API路由
app.use('/api/animal-game', animalGameRouter);

// ==================== 太空挑战游戏 API ====================
// 使用路由文件
app.use('/api/space-game', spaceGameRouter);
...
app.get('/api/health', async (req, res) => {
    try {
        ...
        const gameRoundCount = await GameRound.countDocuments();
...
// 动物大冒险路由
app.get('/animal-game', (req, res) => {
    res.sendFile(path.join(__dirname, '../frontend/animal-game/index.html'));
});
...
// 太空挑战路由
app.get('/space-challenge', (req, res) => {
    res.sendFile(path.join(__dirname, '../frontend/space-challenge/index.html'));
});
...
app.use('/animal-game/css', express.static(path.join(__dirname, '../frontend/animal-game/css')));
app.use('/animal-game/js', express.static(path.join(__dirname, '../frontend/animal-game/js')));
app.use('/animal-game/images', express.static(path.join(__dirname, '../frontend/animal-game/images')));

app.use('/space-challenge/css', express.static(path.join(__dirname, '../frontend/space-challenge/css')));
app.use('/space-challenge/js', express.static(path.join(__dirname, '../frontend/space-challenge/js')));
...
// 太空挑战游戏循环函数
function startSpaceGameLoop() {
    // 开始新一局下注
    currentGame.sessionId = `session_${Date.now()}`;
    currentGame.status = 'betting';
    currentGame.timeRemaining = 30;
    currentGame.userMaxBets = {};
    currentGame.bossStar = null;
    currentGame.fighterStars = [null, null, null, null];
    
    console.log(`[太空挑战] 新一局开始: ${currentGame.sessionId}`);
    ...
    const bettingTimer = setInterval(() => {
        ...
        // 通过WebSocket广播时间更新
        io.to('space-game').emit('space-game-timer', {
...
async function startRevealingPhase() {
    // 下注结束，进入揭晓阶段
    currentGame.status = 'revealing';
    currentGame.timeRemaining = 5;
    console.log(`[太空挑战] 进入揭晓阶段`);

    // 生成所有星级
    const { generateBossStar } = require('./utils/space-game-economy');
    currentGame.bossStar = generateBossStar();
    for (let i = 0; i < 4; i++) {
        currentGame.fighterStars[i] = Math.floor(Math.random() * 11);
    }
    
    // 结算所有下注
    await calculateSpaceGameResults();
    
    // 设置5秒后开始新一局
    const revealingTimer = setInterval(() => {
        currentGame.timeRemaining--;
        ...
        if (currentGame.timeRemaining <= 0) {
            clearInterval(revealingTimer);
            
            // 调整经济
            gameEconomyState.currentRound++;
            if (gameEconomyState.currentRound >= gameEconomyState.cycleLength) {
                adjustBossEconomy(gameEconomyState.totalProfitLoss);
                gameEconomyState.currentRound = 0;
                gameEconomyState.totalProfitLoss = 0;
            }
...
async function calculateSpaceGameResults() {
    try {
        const bets = await SpaceGameBet.find({ 
        ...
        for (const bet of bets) {
            const fighterIndex = bet.betOnFighter - 1;
            const fighterStar = currentGame.fighterStars[fighterIndex];
            const bossStar = currentGame.bossStar;

            let isWin = false;
            let profit = 0;

            if (fighterStar > bossStar) {
                isWin = true;
                profit = bet.betAmount * 2;
                totalSystemProfit -= bet.betAmount;
            } else if (fighterStar === bossStar) {
                isWin = true;
            ...
            bet.profit = profit - bet.betAmount;
            bet.bossStar = bossStar;
            bet.playerFighterStar = fighterStar;
            await bet.save();
            ...
            if (isWin) {
                ...
                // 通过WebSocket通知用户
                io.to(`user_${user._id}`).emit('space-game-win', {
                    amount: profit,
                    newPoints: user.points,
                    fighterStar: fighterStar,
                    bossStar: bossStar
                });
            }
        }

        gameEconomyState.totalProfitLoss += totalSystemProfit;
        console.log(`[太空挑战] 本局系统盈亏: ${totalSystemProfit} 积分`);
...
server.listen(PORT, async () => {
    ...
    // 修复：立即启动动物大冒险游戏引擎
    console.log('🎮 启动动物大冒险游戏引擎...');
    gameEngine.start();
    
    // 启动太空挑战游戏
    console.log('🚀 启动太空挑战游戏...');
    setTimeout(startSpaceGameLoop, 2000); // 延迟2秒启动
    
    console.log('========================================');
    console.log(`🎮 天创娃娃收藏服务器运行在端口 ${PORT}`);
    console.log(`🌐 WebSocket已启用`);
    console.log(`📊 数据库: ${mongoose.connection.readyState === 1 ? '✅ 已连接' : '❌ 未连接'}`);
    console.log(`🎮 动物大冒险游戏已启动`);
    console.log(`🚀 太空挑战游戏已启动`);
    console.log('========================================');
});
