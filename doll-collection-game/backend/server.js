...
try {
    ...
    console.error('❌ 数据库模型加载失败:', error.message);
}
...
const createDefaultAdmin = async () => {
    try {
        if (!User) {
            console.log('⚠️ User模型未加载，跳过创建管理员');
            return;
        }
...
const authenticateToken = async (req, res, next) => {
    ...
    try {
        ...
        if (!User) {
            return res.status(500).json({ message: '数据库模型未加载' });
        }
...
app.get('/api/health', async (req, res) => {
    try {
        ...
        if (User) userCount = await User.countDocuments();
        if (Doll) dollCount = await Doll.countDocuments();
    ...
    if (req.path.startsWith('/api/')) {
        return res.status(404).json({
            ...
            availableRoutes: [
                ...
                'GET /api/dolls/my-dolls',
                'POST /api/dolls/buy',
                'POST /api/dolls/synthesize',
