...
// 系统配置
let systemConfig = {
    dollPrices: { 1: 50, 2: 210, 3: 500 }, // 修改二级娃娃价格为210
...
// 每日收益自动发放任务 - 每天24:00执行
cron.schedule('0 0 * * *', async () => {
    console.log('🕛 开始执行每日收益发放...');
    try {
        const activeDolls = await Doll.find({ active: true }).populate('userId');
        ...
        for (const doll of activeDolls) {
            ...
            // 更新用户积分
            await User.findByIdAndUpdate(doll.userId._id, {
                $inc: { points: income }
            });
            
            // 记录交易
            await Transaction.create({
                userId: doll.userId._id,
                type: 'income',
                amount: income,
        ...
        console.log(`✅ 每日收益发放完成，共发放 ${totalDistributed.toFixed(2)} 积分`);
    } catch (error) {
...
// 批量购买娃娃
app.post('/api/dolls/buy-multiple', authenticateToken, async (req, res) => {
    try {
        ...
        await Transaction.create({
            ...
            type: 'purchase',
            amount: -totalPrice,
            description: `批量购买${quantity}个${level}级娃娃`
        });
...
app.post('/api/dolls/synthesize', authenticateToken, async (req, res) => {
    try {
        ...
        if (isSuccess) {
            ...
            const newLifespan = systemConfig.dollLifespans[newLevel];
            
            // 动态计算剩余天数：基于材料娃娃的寿命比例
            const doll1Ratio = doll1.remainingDays / doll1.lifespan;
            const doll2Ratio = doll2.remainingDays / doll2.lifespan;
            const avgRatio = (doll1Ratio + doll2Ratio) / 2;
            
            // 如果两个娃娃都是满寿命，新娃娃也应该是满寿命
            let remainingDays;
            if (doll1.remainingDays === doll1.lifespan && doll2.remainingDays === doll2.lifespan) {
                remainingDays = newLifespan;
            } else {
                remainingDays = Math.round(newLifespan * avgRatio);
            }
...
// 获取筛选的娃娃列表
app.get('/api/admin/dolls-filtered', authenticateToken, requireAdmin, async (req, res) => {
    try {
        const { level, userId, active, page = 1, limit = 50 } = req.query;
        ...
        const dolls = await Doll.find(filter)
            .populate('userId', 'username')
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(parseInt(limit));
            
        const total = await Doll.countDocuments(filter);
...
// 获取筛选的交易记录
app.get('/api/admin/transactions-filtered', authenticateToken, requireAdmin, async (req, res) => {
    try {
...
// 更新娃娃信息
app.put('/api/admin/dolls/:id', authenticateToken, requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { remainingDays, active, dailyIncome } = req.body;
        ...
        if (remainingDays !== undefined) doll.remainingDays = parseInt(remainingDays);
        if (active !== undefined) doll.active = active;
        if (dailyIncome !== undefined) doll.dailyIncome = parseFloat(dailyIncome);
        
        await doll.save();
        
        res.json({ message: '娃娃信息更新成功', doll });
    } catch (error) {
        console.error('更新娃娃信息错误:', error);
        res.status(500).json({ message: '服务器错误' });
    }
});

// 删除娃娃
app.delete('/api/admin/dolls/:id', authenticateToken, requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        
        const doll = await Doll.findByIdAndDelete(id);
        if (!doll) {
            return res.status(404).json({ message: '娃娃不存在' });
        }
...
app.post('/api/admin/update-doll-prices', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        res.json({ message: '娃娃价格更新成功', config: systemConfig });
    } catch (error) {
...
app.post('/api/admin/calculate-daily-income', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        res.json({ 
            message: '每日收益计算完成',
            totalIncome: incomeRecords.reduce((sum, record) => sum + record.income, 0),
