...
// 动物大冒险游戏状态管理
let currentAdventureRound = null;
let adventureRoundTimer = null;
...
io.on('connection', (socket) => {
    ...
    // 动物大冒险游戏加入
    socket.on('join-adventure', () => {
        socket.join('animal-adventure');
        
        // 如果没有当前轮次，开始新轮次
        if (!currentAdventureRound) {
            startNewAdventureRound();
        }
        
        socket.emit('adventure-status', {
            roundId: currentAdventureRound.roundId,
            status: currentAdventureRound.status,
            timeLeft: currentAdventureRound.status === 'active' ? 
                Math.max(0, ADVENTURE_CONFIG.ROUND_DURATION - (Date.now() - currentAdventureRound.startTime)) : 0
        });
    });
...
// 开始新轮次
async function startNewAdventureRound() {
    try {
        const lastRound = await GameRound.findOne().sort({ roundId: -1 });
        const roundId = lastRound ? lastRound.roundId + 1 : 1;
        
        currentAdventureRound = new GameRound({
            roundId,
            status: 'active',
            startTime: new Date()
        });
        
        await currentAdventureRound.save();
        
        // 设置定时器结束轮次
        adventureRoundTimer = setTimeout(async () => {
            await endAdventureRound();
        }, ADVENTURE_CONFIG.ROUND_DURATION);
        
        // 通知所有游戏玩家
        io.to('animal-adventure').emit('adventure-new-round', {
            roundId: currentAdventureRound.roundId,
            status: 'active',
            duration: ADVENTURE_CONFIG.ROUND_DURATION
        });
        
        console.log(`🎮 动物大冒险第${roundId}轮开始`);
    } catch (error) {
        console.error('❌ 开始动物大冒险轮次失败:', error);
    }
}

// 结束轮次
async function endAdventureRound() {
    if (!currentAdventureRound) return;
    
    try {
        currentAdventureRound.status = 'finished';
        currentAdventureRound.endTime = new Date();
        
        // 确定猎人袭击的房间
        const isRageMode = Math.random() < ADVENTURE_CONFIG.RAGE_MODE_CHANCE;
        const numRooms = isRageMode ? 
            Math.floor(Math.random() * 4) + 1 : 1;
        
        const hunterRooms = [];
        const availableRooms = [...ADVENTURE_CONFIG.ROOMS];
        ...
        currentAdventureRound.hunterRooms = hunterRooms;
        currentAdventureRound.isRageMode = isRageMode;
        
        // 获取所有下注
        const allBets = await PlayerBet.find({ roundId: currentAdventureRound.roundId });
        ...
        const serviceFee = totalLoserBets * ADVENTURE_CONFIG.SERVICE_FEE;
        ...
        // 更新统计数据
        await updateAdventureGameStats();
        
        // 通知前端
        io.to('animal-adventure').emit('adventure-round-end', {
            roundId: currentAdventureRound.roundId,
            hunterRooms: hunterRooms,
            isRageMode: isRageMode,
            results: allBets.map(bet => ({
                userId: bet.userId,
                room: bet.room,
                isSurvivor: bet.isSurvivor,
                reward: bet.reward,
                candies: bet.candies
            }))
        });
        ...
        console.log(`🎮 动物大冒险第${currentAdventureRound.roundId}轮结束，猎人袭击了: ${hunterRooms.join(', ')}`);
    } catch (error) {
        console.error('❌ 结束动物大冒险轮次失败:', error);
    }
}

// 更新游戏统计
async function updateAdventureGameStats() {
    try {
        ...
        stats.totalRounds++;
        stats.totalPlayers += currentAdventureRound.totalPlayers;
        stats.totalBets += currentAdventureRound.totalBets;
        
        // 更新房间统计
        const roomBets = await PlayerBet.aggregate([
            { $match: { roundId: currentAdventureRound.roundId } },
        ...
        console.error('❌ 更新动物大冒险游戏统计失败:', error);
    }
}
...
// 获取当前游戏状态
app.get('/api/animal-adventure/status', authenticateToken, async (req, res) => {
    try {
        if (!currentAdventureRound) {
            await startNewAdventureRound();
        }
        ...
        res.json({
            ...
            userCandies: req.user.candies || 0
        });
    } catch (error) {
...
// 下注
app.post('/api/animal-adventure/bet', authenticateToken, async (req, res) => {
    try {
        const { room, betAmount } = req.body;
        
        if (!currentAdventureRound || currentAdventureRound.status !== 'active') {
        ...
        // 更新轮次统计
        currentAdventureRound.totalBets += betAmount;
        currentAdventureRound.totalPlayers = await PlayerBet.distinct('userId', { 
            roundId: currentAdventureRound.roundId 
...
// 获取历史记录
app.get('/api/animal-adventure/history', authenticateToken, async (req, res) => {
    try {
        const history = await PlayerBet.find({ userId: req.user._id })
            .populate('roundId')
            .sort({ timestamp: -1 })
            .limit(10);
...
// 动物大冒险游戏页面路由
app.get('/animal-adventure', (req, res) => {
    res.sendFile(path.join(__dirname, '../frontend/animal-adventure.html'));
});
