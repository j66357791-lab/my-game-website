...
app.post('/api/dolls/synthesize', authenticateToken, async (req, res) => {
    try {
        ...
        if (isSuccess) {
            ...
            const bonusDays = Math.floor(baseLifespan * 0.2);
...
// 回收娃娃
app.post('/api/dolls/recycle/:dollId', authenticateToken, async (req, res) => {
    try {
        ...
        try {
            // 返还积分
            await User.findByIdAndUpdate(req.user._id, {
                $inc: { points: parseFloat(refundAmount) }
            });
            ...
            await Transaction.create([{
                ...
                description: `回收${doll.level}级娃娃(ID: ${dollId})`
            }], { session });
            ...
            res.json({ 
                message: `回收成功，返还${refundAmount}积分`,
