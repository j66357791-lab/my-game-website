            ...
            for (const room of winnerRooms) {
                for (const player of room.players) {
                    try {
                        ...
                        // 修复：先检查用户是否存在
                        const userExists = await User.findById(userIdStr);
                        if (!userExists) {
                            console.error(`❌ 用户不存在: ${userIdStr}`);
                            continue;
                        }
                        
                        console.log(`🎉 用户当前积分: ${userExists.points}`);
                        
                        // 修复：确保积分正确返还 - 使用事务确保数据一致性
