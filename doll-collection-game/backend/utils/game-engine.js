  ...
  async endRound() {
    ...
    try {
      ...
      const targetCount = isRageMode ? 
        Math.floor(Math.random() * 4) + 1 : 1; // 狂暴模式1-4个房间
      
      // 获取有玩家的房间
      const roomsWithPlayers = this.currentRound.rooms.filter(room => room.players.length > 0);
      ...
      // 随机选择目标房间
      const targets = [];
      const availableRooms = [...roomsWithPlayers];
