...
class AnimalGameEngine {
    ...
    async calculateResults(targets) {
        ...
        console.log(`🏆 幸存房间: ${winnerRooms.map(r => r.roomName).join('、')}`);
        ...
        for (const room of loserRooms) {
            for (const player of room.players) {
                ...
                // 修复：确保糖果正确保存
                const updateResult = await GameStats.findOneAndUpdate(
                ...
                console.log(`💔 糖果更新结果:`, updateResult);
        ...
        // 修复：处理幸存者 - 返还本金 + 瓜分奖金池 - 扣除手续费
        if (totalWinnerBet > 0 && prizePool > 0) {
            console.log(`🎉 开始处理 ${winnerRooms.reduce((sum, room) => sum + room.players.length, 0)} 名幸存者积分返还`);
            ...
            for (const room of winnerRooms) {
                for (const player of room.players) {
                    try {
                        ...
                        console.log(`🎉 处理幸存者 ${userIdStr} (${player.userId.username})`);
                        
                        // 修复：100%返还本金（投注时已扣除，现在要返还）
                        ...
                        console.log(`🎉 积分计算: 本金${principalReturn} + 奖金${prizeShare} = 应得${totalShouldReceive} - 手续费${fee} = 实际获得${actualReceive}`);
                        ...
                        try {
                            // 再次检查用户积分
                            const freshUser = await User.findById(userIdStr).session(session);
                            if (!freshUser) {
                                throw new Error('用户不存在');
                            }
                            
                            console.log(`🎉 用户当前积分: ${freshUser.points}`);
                            ...
                            console.log(`🎉 积分更新成功: ${freshUser.points} -> ${updatedUser.points} (+${actualReceive})`);
                            ...
                            await Transaction.create([{
                                ...
                                description: `动物大冒险获胜 - 第${this.currentRound.roundNumber}轮 (本金:${principalReturn}+奖金:${prizeShare}-手续费:${fee})`,
                                timestamp: new Date()
                            }], { session: session });
                            
                            // 更新统计
                            const statsUpdateResult = await GameStats.findOneAndUpdate(
                                { userId: userIdStr },
                                { 
                                    $inc: { 
                                        wins: 1,
                            ...
                            console.log(`🎉 统计更新成功:`, statsUpdateResult);
                            ...
                            winnerResults.push({
                                ...
                                fee,
                                actualReceive,
                                oldPoints: freshUser.points,
                                newPoints: updatedUser.points
                            });
                            
                        } catch (transactionError) {
                            console.error(`❌ 处理幸存者 ${userIdStr} 事务错误:`, transactionError);
            ...
            console.log(`⚠️ 没有幸存者或奖金池为空，跳过积分返还`);
        }
