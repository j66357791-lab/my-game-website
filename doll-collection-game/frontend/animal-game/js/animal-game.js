class AnimalGame {
    ...
    async refreshUserInfo() {
        try {
            ...
            if (response.ok) {
                ...
                this.updateBalance();
                console.log('🔄 刷新用户信息成功:', this.currentUser);
            }
        } catch (error) {
    ...
    async processUserResult(data) {
        ...
        if (userInWinnerRoom) {
            ...
            if (winnerResult) {
                message = `🎉 恭喜幸存！获得 ${winnerResult.actualReceive} 积分（本金${winnerResult.principalReturn}+奖金${winnerResult.prizeShare}-手续费${winnerResult.fee}）`;
            } else {
                message = '🎉 恭喜幸存！';
            }
        } else {
