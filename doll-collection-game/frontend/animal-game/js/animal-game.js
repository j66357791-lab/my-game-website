class AnimalGame {
    ...
    async getCurrentUser() {
        // 这里需要根据你的系统实现获取当前用户
        // 可能是从cookie、localStorage或全局变量获取
        try {
            // 示例：从localStorage获取
    ...
    async loadUserStats() {
        try {
            ...
            document.getElementById('userCandies').textContent = stats.candies || 0;
    ...
    selectRoom(roomName) {
        ...
        document.querySelector(`[data-room="${roomName}"]`).classList.add('selected');
    ...
    async placeBet() {
        ...
        try {
            ...
            if (result.success) {
                ...
                // 显示取消按钮
                document.getElementById('cancelBet').style.display = 'inline-block';
    ...
    async cancelBet() {
        ...
        document.getElementById('cancelBet').style.display = 'none';
        document.querySelectorAll('.room-card').forEach(card => {
            card.classList.remove('selected');
        });
    ...
    handleNewRound(data) {
        ...
        document.getElementById('timerValue').textContent = '40';
        document.getElementById('timerStatus').textContent = '游戏进行中';
    ...
    handleGameUpdate(data) {
        ...
        if (data.rooms) {
            for (const room of data.rooms) {
                const userInRoom = room.players.find(p => p.userId._id === this.currentUser._id);
                if (userInRoom) {
                    this.hasBet = true;
                    this.selectedRoom = room.roomName;
                    this.currentBet = userInRoom.betAmount;
                    
                    document.querySelector(`[data-room="${room.roomName}"]`).classList.add('selected');
                    document.getElementById('cancelBet').style.display = 'inline-block';
                    break;
                }
            }
    ...
    showResult(data) {
        ...
        // 停止计时器
        this.stopTimer();
        document.getElementById('timerStatus').textContent = '本轮结束';
    }
    ...
    updateBalance() {
        document.getElementById('userBalance').textContent = this.currentUser.points || 0;
    }
