class AnimalGame {
    ...
    selectRoom(roomName) {
        // 修复：允许用户自由选择房间，即使已经投注
        // 但是如果已经投注，只显示提示，不阻止选择
        if (this.hasBet) {
    ...
    updateBetButton() {
        ...
        if (betBtn) {
            // 修复：如果已经投注，显示"重新投注"而不是禁用
            const canBet = this.selectedRoom && this.currentBet > 0;
        ...
        if (data.rooms) {
            ...
            for (const room of data.rooms) {
                ...
                if (userInRoom) {
                    ...
                    // 修复：不自动选择房间，让用户自由选择
                    console.log(`📍 用户已在 ${room.roomName} 投注 ${userInRoom.betAmount} 积分，但不自动选择房间`);
