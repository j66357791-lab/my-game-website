    ...
    async calculateDailyIncome() {
        ...
        try {
            ...
            for (const user of allUsers) {
                ...
                try {
                    // 获取用户的娃娃数据
                    const dollsResponse = await fetch(`${CONFIG.API_BASE}/dolls/my-dolls`, {
                        headers: {
                            'Authorization': `Bearer ${localStorage.getItem('token')}`
                        }
                    });
                    ...
                    if (dollsResponse.ok) {
                        const dollsData = await dollsResponse.json();
                        const userDolls = Array.isArray(dollsData.dolls) ? dollsData.dolls : [];
                    ...
                    console.error(`获取用户 ${user.username} 的娃娃数据失败:`, error);
                }
            }
