...
const AdminManager = {
    ...
    async loadAdminData() {
        ...
        try {
            ...
            // 获取所有娃娃
            const dollsResponse = await fetch(`${this.API_BASE}/dolls`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            });
            ...
            // 获取交易记录
            const transactionsResponse = await fetch(`${this.API_BASE}/transactions`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            });
    ...
    exportData(type) {
        ...
        window.open(`${this.API_BASE}/export/${type}?token=${token}`, '_blank');
    }
};
