    ...
    updateUserStats() {
        ...
        if (userDolls && Array.isArray(userDolls)) {
            userDolls.forEach(doll => {
                if (doll.active && doll.dailyIncome) {
                    const income = parseFloat(doll.dailyIncome) || 0;
        ...
        statsContainer.innerHTML = `
            ...
            <div class="stat-item">
                ...
                <div class="stat-info">
                    <span class="stat-value">${totalDailyIncome.toFixed(2)}</span>
        ...
        // 🔧 修复：更新分级收益统计
        const level1IncomeElement = document.getElementById('level1-income');
        const level2IncomeElement = document.getElementById('level2-income');
        const level3IncomeElement = document.getElementById('level3-income');
        
        if (level1IncomeElement) {
            level1IncomeElement.textContent = level1Income.toFixed(2);
        }
        if (level2IncomeElement) {
            level2IncomeElement.textContent = level2Income.toFixed(2);
        }
        if (level3IncomeElement) {
            level3IncomeElement.textContent = level3Income.toFixed(2);
        }
        
        // 更新预计发放金额
        const expectedPayoutElement = document.getElementById('expected-payout');
        if (expectedPayoutElement) {
            expectedPayoutElement.textContent = totalDailyIncome.toFixed(2);
        }
    },
