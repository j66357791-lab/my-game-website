...
const SynthesisManager = {
    ...
    calculateSynthesizedLifespan(doll1, doll2, newLevel) {
        ...
        // 计算新娃娃的寿命
        // 方案1：使用材料娃娃剩余天数的平均值，但不超过基础寿命
        const calculatedLifespan = Math.min(avgRemainingDays, baseLifespans[newLevel]);
        
        // 方案2：如果材料娃娃都是满寿命，给新娃娃满寿命
        const bothFullLifespan = doll1.remainingDays === doll1.lifespan && 
                                  doll2.remainingDays === doll2.lifespan;
        ...
        // 方案3：否则使用计算值
        return Math.max(calculatedLifespan, 30); // 最低保证30天寿命
    },
        ...
        try {
            const response = await fetch(`${AppState.API_BASE}/dolls/synthesize`, {
                ...
                body: JSON.stringify({
                    ...
                    points,
                    // 传递计算好的寿命信息给后端
                    synthesizedLifespan: this.calculateSynthesizedLifespan(doll1, doll2, doll1.level + 1)
                })
            });
