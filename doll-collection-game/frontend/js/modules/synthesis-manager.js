...
const SynthesisManager = {
    // 🔧 修复：强制刷新娃娃数据
    async refreshUserDolls() {
        try {
            console.log('强制刷新用户娃娃数据...');
            ...
            if (response.ok) {
                ...
                console.log('娃娃数据刷新完成:', data.dolls);
                return data.dolls || [];
            } else {
                console.error('刷新娃娃数据失败:', response.status);
                return AppState.userDolls || [];
            }
        } catch (error) {
            console.error('刷新娃娃数据错误:', error);
            return AppState.userDolls || [];
        }
    },

    // 🔧 修复：更新可用娃娃列表（强制刷新数据）
    async updateAvailableDolls() {
        // 先刷新数据
        await this.refreshUserDolls();
    ...
    // 选择娃娃用于合成
    async selectDollForSynthesis(slot) {
        // 先刷新数据
        await this.refreshUserDolls();
    ...
    // 从列表中选择娃娃用于合成
    async selectDollForSynthesisFromList(dollId) {
        // 先刷新数据
        await this.refreshUserDolls();
