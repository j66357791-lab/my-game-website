...
function initEventListeners() {
    ...
    if (transferAmount) {
        transferAmount.addEventListener('input', updateTransferFeeDisplay);
    }
    
    const searchUsername = document.getElementById('search-username');
    if (searchUsername) {
        searchUsername.addEventListener('input', debounceSearchUser);
    }
    
    const merchantAppointmentForm = document.getElementById('merchant-appointment-form');
    if (merchantAppointmentForm) {
        merchantAppointmentForm.addEventListener('submit', handleMerchantAppointment);
    }
...
// ✅ 新增：复制用户ID
function copyUserId() {
    const userId = document.getElementById('recipient-id-display').textContent;
    if (userId && userId !== '请选择接收人') {
...
async function searchUser(username) {
    ...
    try {
        const response = await fetch(`${API_BASE}/users/search?query=${encodeURIComponent(username)}`, {
            headers: {
        ...
        if (response.ok && data.users && data.users.length > 0) {
            ...
            showNotification('未找到该用户', 'warning');
        }
    } catch (error) {
...
function displaySearchResults(users) {
    ...
    users.forEach(user => {
        ...
        userItem.innerHTML = `
            ...
            <button class="btn btn-sm btn-primary" onclick="selectRecipient('${user._id}', '${user.username}')">选择</button>
        `;
...
function updateTransferInfo() {
    ...
    if (currentUser.role === 'user') {
        fee = Math.floor(amount * 0.05); // 普通用户5%手续费
    } else if (currentUser.role === 'merchant') {
        fee = 0; // 商人0手续费
    }
    
    // 这里需要查询接收者角色来计算奖励，暂时简化处理
    const actualAmount = amount - fee;
    const totalDeduction = amount;
    
    feeDisplay.textContent = `${fee} 积分 (${currentUser.role === 'user' ? '5%' : '0%'})`;
    bonusDisplay.textContent = '待确认接收者身份';
...
async function executeTransfer() {
    ...
    const description = descriptionInput.value || '积分转增';
    
    if (!recipientId || !amount || amount <= 0) {
        alert('请输入有效的接收者ID和转账金额');
        return;
    }
    ...
    try {
        ...
        if (response.ok) {
            alert(`转账成功！${data.transfer.recipientUsername} 收到 ${data.transfer.actualAmount} 积分`);
            
            // 更新用户积分
            currentUser.points = data.transfer.senderNewBalance;
            updateUI();
...
async function loadTransferRecords() {
    ...
    try {
        const response = await fetch(`${API_BASE}/transactions/transfer-records`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
        });
        ...
        if (response.ok) {
            const data = await response.json();
            updateTransferRecordsTable(Array.isArray(data.records) ? data.records : []);
        } else {
            console.error('获取转增记录失败');
            updateTransferRecordsTable([]);
        }
    } catch (error) {
...
// ✅ 新增：更新转增记录表格
function updateTransferRecordsTable(records) {
    ...
    records.forEach(record => {
        ...
        row.innerHTML = `
            ...
            <td>${record.amount || 0} 积分</td>
            <td>${record.transferData ? record.transferData.fee || 0} 积分</td>
            <td>${record.transferData ? record.transferData.bonus || 0} 积分</td>
            <td>${record.transferData ? record.transferData.actualAmount || 0} 积分</td>
        `;
        ...
        row.innerHTML = `
            ...
            <td>
                ...
                <button class="btn btn-sm ${user.role === 'merchant' ? 'btn-warning' : 'btn-secondary'}" 
                        onclick="${user.role === 'merchant' ? 'revokeMerchant' : 'appointMerchant'}('${user._id}')">
...
// ✅ 新增：任命商人
async function appointMerchant(userId) {
    const user = allUsers.find(u => u._id === userId);
    if (!user) return;
    
    if (!confirm(`确定要任命用户 "${user.username}" 为商人吗？`)) {
        return;
    }
    
    try {
        const response = await fetch(`${API_BASE}/admin/appoint-merchant`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('token')}`
...
// ✅ 新增：撤销商人
async function revokeMerchant(userId) {
    const user = allUsers.find(u => u._id === userId);
    if (!user) return;
    
    if (!confirm(`确定要撤销用户 "${user.username}" 的商人身份吗？`)) {
        return;
    }
    
    try {
        const response = await fetch(`${API_BASE}/admin/revoke-merchant`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('token')}`
...
function updateLastPayoutTime() {
    ...
    if (element) {
        element.textContent = lastPayoutTime ? lastPayoutTime.toLocaleString() : '从未发放';
    }
}
...
function updatePayoutHistory(amount) {
    ...
    historyItem.innerHTML = `
        <div class="payout-time">${new Date().toLocaleString()}</div>
        ...
        backpackItem.innerHTML = `
            ...
            <div class="backpack-item-body">
                ...
                <div class="backpack-item-info">
                    ...
                    <div class="backpack-item-info-item">
                        <span>${doll.purchaseDate ? new Date(doll.purchaseDate).toLocaleDateString() : '未知日期'}</span>
                    </div>
                </div>
...
function updateBackpackStats(total, filtered) {
    ...
    if (valueElement) {
        const totalValue = userDolls.reduce((sum, doll) => {
            return sum + (doll.level === 1 ? 50 : doll.level === 2 ? 200 : doll.level === 3 ? 500 : 0);
        }, 0);
...
style.textContent = `
    ...
    .backpack-item:nth-child(1) { animation-delay: 0.05s; }
    .backpack-item:nth-child(2) { animation-delay: 0.1s; }
    .backpack-item:nth-child(3) { animation-delay: 0.15s; }
    .backpack-item:nth-child(4) { animation-delay: 0.2s; }
    .backpack-item:nth-child(5) { animation-delay: 0.25s; }
    .backpack-item:nth-child(6) { animation-delay: 0.3s; }
`;
