...
function initEventListeners() {
    ...
    // MODIFIED: 更新导航选择器 - 适配底部导航栏
    const navLinks = document.querySelectorAll('.bottom-nav .nav-item');
    navLinks.forEach(link => {
        link.addEventListener('click', handleNavClick);
    });
    
    // MODIFIED: 新增首页快捷卡片事件监听
    const actionCards = document.querySelectorAll('.action-card');
    actionCards.forEach(card => {
        card.addEventListener('click', handleNavClick);
    });
    
    // MODIFIED: 新增子导航事件监听
    const subNavItems = document.querySelectorAll('.sub-nav-item');
    subNavItems.forEach(item => {
        item.addEventListener('click', handleSubNavClick);
    });
...
function handleNavClick(e) {
    ...
    // 🎮 太空挑战链接处理 - 修复！
    if (href && (href.includes('space-game') || href.includes('space-game/'))) {
        console.log('🚀 检测到太空挑战链接');
        ...
        console.log('✅ 用户已登录，跳转到太空挑战');
        
        // 直接跳转，让太空挑战页面自己处理token验证
    ...
    // 🎮 动物大冒险链接处理
    if (href && (href.includes('animal-game') || href.includes('animal-game/'))) {
        console.log('🎮 检测到动物大冒险链接');
        ...
        console.log('✅ 用户已登录，跳转到动物大冒险');
        
        // 直接跳转，让动物大冒险页面自己处理token验证
        window.location.href = href;
        return;
    }
...
// MODIFIED: 新增子导航点击处理函数
function handleSubNavClick(e) {
    const targetTabId = this.getAttribute('data-tab');
    if (!targetTabId) return;
    
    // 获取当前子导航容器
    const subNavContainer = this.closest('.sub-nav') || this.closest('.friends-tabs');
    if (!subNavContainer) return;
    
    // 移除同组内所有标签和内容的激活状态
    const tabs = subNavContainer.querySelectorAll('[class*="tab"], [class*="Tab"]');
    const contents = subNavContainer.parentElement.querySelectorAll('.tab-content, .friends-content');
...
async function handleLogin(e) {
    ...
    try {
        ...
        if (response.ok) {
            // 修复：同时存储到 token 和 userToken
            localStorage.setItem('token', data.token);
            localStorage.setItem('userToken', data.token); // 为太空挑战游戏准备
...
function handleLogout() {
    localStorage.removeItem('token');
    localStorage.removeItem('userToken'); // 清除太空挑战游戏的token
...
function updateUI() {
    if (currentUser) {
        ...
        if (currentUser.role === 'admin') {
            ...
            console.log('✅ 显示管理员导航栏');
        } else {
            // 显示普通用户导航栏
            if (userNav) userNav.style.display = 'flex';
            if (adminNav) adminNav.style.display = 'none';
            console.log('✅ 显示普通用户导航栏');
        }
...
// MODIFIED: 修复好友标签页切换功能
function switchFriendsTab(tabName) {
    console.log('🔄 切换好友标签页:', tabName);
    ...
    // 激活当前点击的标签
    const activeTab = document.querySelector(`[onclick="switchFriendsTab('${tabName}')"]`);
    if (activeTab) {
    ...
    if (targetContent) {
        targetContent.classList.add('active');
        console.log('✅ 好友内容区域激活:', `friends-${tabName}`);
    } else {
        console.error('❌ 找不到好友内容区域:', `friends-${tabName}`);
    }
}
...
async function loadAdminData() {
    ...
    try {
        // 并行加载所有数据
        const [usersData, dollsData, transactionsData, configData] = await Promise.all([
            ...
            fetch(`${API_BASE}/admin/dolls`, {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
            }).then(res => res.ok ? res.json() : { dolls: [] }),
...
async function loadAdminDolls() {
    console.log('🔄 加载管理员娃娃数据...');
    
    try {
        const response = await fetch(`${API_BASE}/admin/dolls`, {
            headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        });
        
        if (response.ok) {
            const data = await response.json();
            allDolls = Array.isArray(data.dolls) ? data.dolls : [];
            updateDollsTable(allDolls);
            console.log('✅ 娃娃数据更新完成');
        }
    } catch (error) {
...
function updateAdminStats() {
    ...
    const activeUsersToday = allUsers.filter(user => 
        user.active && user.lastLogin && new Date(user.lastLogin) >= todayStart
    ).length;
    
    const totalActiveUsers = allUsers.filter(user => user.active).length;
    
    const userRetentionRate = allUsers.length > 0 ? 
        ((totalActiveUsers / allUsers.length) * 100).toFixed(1) : 0;
    
    // 计算娃娃统计
    const level1Dolls = allDolls.filter(doll => doll.level === 1).length;
    const level2Dolls = allDolls.filter(doll => doll.level === 2).length;
    const level3Dolls = allDolls.filter(doll => doll.level === 3).length;
    
    const synthesisSuccessRate = allTransactions.filter(t => t.type === 'synthesis').length > 0 ?
        ((allTransactions.filter(t => t.type === 'synthesis' && t.success).length / 
              allTransactions.filter(t => t.type === 'synthesis').length) * 100).toFixed(1) : 0;
    ...
    const todayRevenue = allTransactions
        .filter(t => t.createdAt && new Date(t.createdAt) >= todayStart && t.amount > 0)
        .reduce((sum, t) => sum + t.amount, 0);
    
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
    const monthTransactions = allTransactions.filter(t => 
        t.createdAt && new Date(t.createdAt) >= monthStart
    ).length;
    
    const monthRevenue = allTransactions
        .filter(t => t.createdAt && new Date(t.createdAt) >= monthStart && t.amount > 0)
        .reduce((sum, t) => sum + t.amount, 0);
    ...
    const userStatsElements = {
        ...
        'user-retention-rate': userRetentionRate + '%'
    };
    
    const dollStatsElements = {
        'level1-dolls-count': level1Dolls,
        'level2-dolls-count': level2Dolls,
        'level3-dolls-count': level3Dolls,
        'synthesis-success-rate': synthesisSuccessRate + '%'
    };
    
    const transactionStatsElements = {
        'today-transactions': todayTransactions,
    ...
    // 更新用户统计
    Object.keys(userStatsElements).forEach(id => {
        const element = document.getElementById(id);
        if (element) {
            element.textContent = userStatsElements[id];
        }
    });
    
    // 更新娃娃统计
    Object.keys(dollStatsElements).forEach(id => {
        const element = document.getElementById(id);
        if (element) {
            element.textContent = dollStatsElements[id];
        }
    });
    
    // 更新交易统计
    Object.keys(transactionStatsElements).forEach(id => {
        const element = document.getElementById(id);
        if (element) {
            element.textContent = transactionStatsElements[id];
        }
    });
    
    // 更新仪表盘概览数据
    const dashboardElements = {
        'total-users-count': allUsers.length,
        'total-dolls-count': allDolls.length,
        'total-transactions-count': allTransactions.length,
        'today-income': todayRevenue
    };
    
    Object.keys(dashboardElements).forEach(id => {
        const element = document.getElementById(id);
        if (element) {
            element.textContent = dashboardElements[id];
        }
    });
    
    console.log('✅ 管理员统计数据更新完成');
}

// 修复：初始化管理员后台导航
function initAdminNavigation() {
    console.log('🔄 初始化管理员导航...');
    ...
    function handleMenuClick(e) {
        ...
        const targetPage = this.getAttribute('data-admin-page');
        if (!targetPage) return;
        
        console.log('🔄 切换管理员页面:', targetPage);
        ...
        // 激活当前菜单项和页面
        this.classList.add('active');
        const targetPageElement = document.getElementById(`admin-${targetPage}`);
        if (targetPageElement) {
            targetPageElement.classList.add('active');
        }
        
        // 加载对应页面的数据
        loadAdminPageData(targetPage);
    }
}

// 修复：加载管理员页面数据
function loadAdminPageData(page) {
    console.log('🔄 加载管理员页面数据:', page);
    
    switch(page) {
        case 'dashboard':
            // 仪表盘数据已经在 loadAdminData 和 updateAdminStats 中加载
            break;
        case 'users':
            loadAdminUsers();
            break;
        case 'user-stats':
            // 用户统计数据已经在 updateAdminStats 中计算
            break;
        case 'dolls':
            loadAdminDolls();
            break;
        case 'doll-stats':
            // 娃娃统计数据已经在 updateAdminStats 中计算
            break;
        case 'doll-prices':
            // 价格设置数据在 loadAdminData 中加载
            break;
        case 'transactions':
            // 交易数据已经在 loadAdminData 中加载
            break;
        case 'transaction-stats':
            // 交易统计数据已经在 updateAdminStats 中计算
            break;
        case 'system-settings':
            // 系统设置数据在 loadAdminData 中加载
            break;
...
// ==================== 显示面板 - 适配新的面板ID和导航状态更新 ====================

// MODIFIED: 显示面板 - 适配新的面板ID和导航状态更新
function showPanel(panelId) {
    ...
    // MODIFIED: 更新底部导航链接状态 - 支持两种导航栏
    document.querySelectorAll('.bottom-nav .nav-item').forEach(link => {
        link.classList.remove('active');
    });
    
    // 根据用户角色选择对应的导航栏
    const navSelector = currentUser.role === 'admin' ? '#admin-nav' : '#user-nav';
    const activeLink = document.querySelector(`${navSelector} .nav-item[data-panel="${panelId}"]`);
    if (activeLink) {
        activeLink.classList.add('active');
        console.log('✅ 底部导航链接激活:', panelId);
    }
    
    // MODIFIED: 更新面板数据加载逻辑 - 适配新的面板ID
    if (panelId === 'dolls-panel') {
        // 娃娃版块 - 加载娃娃数据
...
// MODIFIED: 打开合成面板 - 适配新的子导航结构
function openSynthesisPanel() {
    // 先切换到娃娃版块
    showPanel('dolls-panel');
