...
const API_BASE = 'http://localhost:3000/api'; // 改为本地地址
...
// 重置统计信息
function resetStats() {
    document.getElementById('total-dolls').textContent = '0';
    document.getElementById('active-dolls').textContent = '0';
    document.getElementById('daily-income').textContent = '0.00';
    document.getElementById('avg-lifespan').textContent = '0';
    document.getElementById('expected-payout').textContent = '0.00';
    document.getElementById('level1-income').textContent = '0.00';
    document.getElementById('level2-income').textContent = '0.00';
    document.getElementById('level3-income').textContent = '0.00';
}
    ...
    validDolls.forEach(doll => {
        ...
        dollCard.innerHTML = `
            <div class="doll-header">
                ...
                <div class="doll-level">ID: ${doll._id || doll.id}</div>
            </div>
            <div class="doll-body">
    ...
    availableDolls.forEach(doll => {
        ...
        dollCard.innerHTML = `
            ...
            <div class="doll-body">
                ...
                <button class="btn btn-block" onclick="selectDollForSynthesisFromList('${doll._id || doll.id}')">选择</button>
            </div>
        `;
...
function selectDollForSynthesisFromList(dollId) {
    ...
    const doll = userDolls.find(d => (d._id === dollId || d.id === dollId));
    if (!doll) {
        alert('娃娃不存在！');
        return;
    }
...
async function synthesizeDolls() {
    ...
    try {
        const response = await fetch(`${API_BASE}/dolls/synthesize`, {
            ...
            body: JSON.stringify({
                doll1Id: doll1._id || doll1.id,
                doll2Id: doll2._id || doll2.id,
                points
            })
        });
...
function updateUsersTable() {
    ...
    allUsers.forEach(user => {
        ...
        row.innerHTML = `
            ...
            <td>
                <button class="btn btn-sm" onclick="adjustUserPoints('${user._id || user.id}')">调整积分</button>
                <button class="btn btn-sm btn-danger" onclick="toggleUserStatus('${user._id || user.id}')">${user.active ? '禁用' : '启用'}</button>
            </td>
        `;
...
function updateDollsTable(dolls) {
    ...
    dolls.forEach(doll => {
        const row = document.createElement('tr');
        row.innerHTML = `
            <td>${doll._id || doll.id}</td>
            <td>${doll.username || doll.userId || '未知用户'}</td>
            <td>${doll.level || 1}</td>
            ...
            <td>
                <button class="btn btn-sm btn-danger" onclick="deleteDoll('${doll._id || doll.id}')">删除</button>
            </td>
        `;
...
function updateTransactionsTable(transactions) {
    ...
    transactions.forEach(transaction => {
        ...
        row.innerHTML = `
            ...
            <td>${transaction.username || transaction.userId || '未知用户'}</td>
