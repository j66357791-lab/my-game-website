...
async function updateDollPrices() {
    ...
    try {
        ...
        // 注意：你的后端目前没有这个接口，这里只是前端示例
        alert('此功能需要后端支持更新系统配置的接口');
        
        // 当你后端添加了接口后，可以取消注释下面的代码：
        /*
...
async function deleteDoll(dollId) {
    ...
    try {
        // 注意：你的后端目前没有这个接口，这里只是前端示例
        alert(`此功能需要后端支持删除娃娃的接口\n删除${doll.level}级娃娃 (ID: ${dollId.substring(0, 8)}...)`);
