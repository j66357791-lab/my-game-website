...
// 更新娃娃价格显示
function updateDollPrices() {
    if (!systemConfig) return;
    
    const priceElements = {
        'level1-price': systemConfig.dollPrices[1],
        'level2-price': systemConfig.dollPrices[2],
        'level3-price': systemConfig.dollPrices[3]
    };
    
    Object.keys(priceElements).forEach(id => {
        const element = document.getElementById(id);
        if (element) {
            element.value = priceElements[id];
        }
    });
    
    // 更新购买卡片中的价格显示
    const dollCards = document.querySelectorAll('.doll-price');
    if (dollCards.length >= 2) {
        dollCards[0].textContent = `${systemConfig.dollPrices[1]} 积分`;
        dollCards[1].textContent = `${systemConfig.dollPrices[2]} 积分`;
        if (dollCards[2]) {
            dollCards[2].textContent = `${systemConfig.dollPrices[3]} 积分`;
        }
    }
}
...
// 批量购买娃娃
async function buyMultipleDolls(level) {
    if (!currentUser) {
        showLoginModal();
        return;
    }
    ...
    const price = systemConfig?.dollPrices[level] || 50;
    ...
    if (!confirm(`确定要购买${quantity}个${level}级娃娃吗？总共需要${totalPrice}积分。`)) {
        return;
    }
    
    try {
        const response = await fetch(`${API_BASE}/dolls/buy-multiple`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('token')}`
...
async function filterDolls(page = 1) {
    try {
        ...
        const response = await fetch(`${API_BASE}/admin/dolls-filtered?${params}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
        });
        ...
        if (response.ok) {
            const data = await response.json();
            updateDollsTable(data.dolls);
            updatePagination('dolls-pagination', data.pagination, filterDolls);
        }
    } catch (error) {
...
async function filterTransactions(page = 1) {
    try {
        ...
        const userId = document.getElementById('transaction-user-filter')?.value || '';
        ...
        const response = await fetch(`${API_BASE}/admin/transactions-filtered?${params}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
        });
        ...
        if (response.ok) {
            ...
            updatePagination('transactions-pagination', data.pagination, filterTransactions);
        }
    } catch (error) {
        console.error('筛选交易记录错误:', error);
    }
}
...
function updatePagination(containerId, pagination, loadFunction) {
    ...
    if (pagination.current > 1) {
        ...
        prevBtn.onclick = () => loadFunction(pagination.current - 1);
    ...
    for (let i = Math.max(1, pagination.current - 2); i <= Math.min(pagination.pages, pagination.current + 2); i++) {
        ...
        pageBtn.className = `btn btn-sm ${i === pagination.current ? 'btn-primary' : ''}`;
    ...
    if (pagination.current < pagination.pages) {
        ...
        nextBtn.onclick = () => loadFunction(pagination.current + 1);
...
function updateDollsTable(dolls) {
    ...
    dolls.forEach(doll => {
        ...
        row.innerHTML = `
            ...
            <td>
                <button class="btn btn-sm" onclick="editDoll('${doll._id}')">编辑</button>
...
// 编辑娃娃
function editDoll(dollId) {
    // 这里应该获取娃娃信息并填充到编辑模态框
    // 为了简化，直接显示模态框
    document.getElementById('edit-doll-id').value = dollId;
    document.getElementById('edit-doll-modal').style.display = 'flex';
}

// 保存娃娃编辑
async function saveDollEdit() {
    const dollId = document.getElementById('edit-doll-id').value;
    const remainingDays = document.getElementById('edit-doll-remaining-days').value;
    const dailyIncome = document.getElementById('edit-doll-daily-income').value;
    const active = document.getElementById('edit-doll-active').checked;
    ...
    try {
        ...
        if (response.ok) {
            alert('娃娃信息更新成功！');
            closeModal('edit-doll-modal');
            filterDolls(); // 重新加载娃娃列表
        } else {
            alert('更新失败');
        }
    } catch (error) {
        console.error('保存娃娃编辑错误:', error);
        alert('网络错误，请稍后重试');
    }
}
...
async function adjustUserPoints(userId) {
    const newPoints = prompt('请输入新的积分数量:');
...
async function updateDollPrices() {
    ...
    const level1Price = document.getElementById('level1-price')?.value || 50;
    const level2Price = document.getElementById('level2-price')?.value || 210;
    const level3Price = document.getElementById('level3-price')?.value || 500;
    ...
    try {
        const response = await fetch(`${API_BASE}/admin/update-doll-prices`, {
            ...
            body: JSON.stringify({
                dollPrices: {
                    1: parseInt(level1Price),
                    2: parseInt(level2Price),
                    3: parseInt(level3Price)
                }
            })
        });
...
async function calculateDailyIncome() {
    ...
    try {
        ...
        if (response.ok) {
            const data = await response.json();
            alert(`${data.message}\n总发放收益: ${data.totalIncome.toFixed(2)} 积分`);
        } else {
            alert('收益计算失败');
        }
    } catch (error) {
