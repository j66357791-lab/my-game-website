...
function showPanel(panelId) {
    ...
    const activeLink = document.querySelector(`.bottom-nav .nav-item[data-panel="${panelId}"]`);
    if (activeLink) {
...
// MODIFIED: 切换好友标签页 - 使用通用的子导航处理函数
function switchFriendsTab(tabName) {
    // 使用通用的子导航处理逻辑
    const tab = document.querySelector(`[onclick="switchFriendsTab('${tabName}')"]`);
    if (tab) {
