    ...
    async checkInvitePermission() {
        try {
            ...
            if (currentMemberCount >= 10) {
                return {
                    canInvite: false,
                    reason: `家庭已满员（${currentMemberCount}/10人）`
                };
            }
            
            return {
                canInvite: true,
                reason: `可以邀请好友（${currentMemberCount}/10人）`
            };
        } catch (error) {
            console.error('❌ 检查邀请权限错误:', error);
            return {
    ...
    showInviteSuccessDetails(data, friendName) {
        try {
            ...
            if (memberCountElement && this.currentFamily) {
                ...
                memberCountElement.textContent = `${currentCount}/10人`;
            }
            ...
            if (familyDetails) {
                familyDetails.innerHTML = `
                    ...
                    <div class="family-detail-item">
                        ...
                        <div class="family-detail-value">${(this.currentFamily.members || []).length + 1}/10人</div>
                    </div>
                `;
            }
            ...
            const modalHtml = `
                <div id="feed-modal" class="modal" style="display: flex;">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="chicken-info">
                                <h4 id="feed-chicken-name">${this.selectedChicken.name}</h4>
                                <p id="feed-chicken-level">当前等级: Lv.${this.selectedChicken.level}</p>
                                <p id="feed-chicken-growth">当前成长值: ${this.selectedChicken.growthValue || 0}</p>
                                <p id="feed-chicken-next">下一级需要: ${this.getNextLevelRequirement(this.selectedChicken.level, this.selectedChicken.quality)}</p>
                                <p id="feed-chicken-owner">主人: ${this.selectedChicken.ownerId ? this.selectedChicken.ownerId.username : '未知'}</p>
                            </div>
