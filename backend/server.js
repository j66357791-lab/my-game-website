...
app.post('/api/orders/sync', authenticateToken, async (req, res) => {
    try {
        ...
        // 获取云端订单（这里使用游戏记录作为订单）
        const cloudOrders = data.gameRecords.filter(record => record.userId === userId);
...
app.post('/api/backpack/sync', authenticateToken, async (req, res) => {
    try {
        ...
        // 获取云端背包数据
        const cloudBackpack = data.dolls[userId] || [];
        ...
        if (newItems.length > 0) {
            if (!data.dolls[userId]) {
                data.dolls[userId] = [];
            }
            data.dolls[userId].push(...newItems);
            saveData();
        }
