...
class DataAdapter {
    ...
    init() {
        ...
        window.addEventListener('pointsUpdated', (e) => {
            ...
            this.syncPointsToAllUsers(e.detail);
        });
     ...
     * 同步积分到所有用户
     */
    syncPointsToAllUsers(pointsData) {
        try {
            ...
            if (userIndex >= 0) {
                allUsers[userIndex].points = pointsData.newPoints;
                localStorage.setItem('allUsers', JSON.stringify(allUsers));
                console.log('数据适配器：已同步积分到allUsers', pointsData.newPoints);
            }
        } catch (error) {
            console.error('数据适配器：同步积分失败', error);
        }
    }
    ...
    setUseLocalStorage(useLocalStorage) {
        ...
        console.log(`数据适配器：数据源切换 ${oldMode ? '本地存储' : '远程API'} -> ${useLocalStorage ? '本地存储' : '远程API'}`);
    ...
    async getUserInfo() {
        try {
            ...
            console.error('数据适配器：获取用户信息失败', error);
            return null;
        }
    }
    ...
    async updateUserInfo(userData) {
        try {
            ...
            console.error('数据适配器：更新用户信息失败', error);
            throw error;
        }
    }
    ...
    async logout() {
        try {
            ...
            console.error('数据适配器：退出登录失败', error);
        }
    }
    ...
    async getPointsBalance() {
        try {
            ...
            console.error('数据适配器：获取积分余额失败', error);
            return 0;
        }
    }
    ...
    async updatePoints(amount, reason, metadata = {}) {
        try {
            ...
            console.error('数据适配器：更新积分失败', error);
            throw error;
        }
    }
    ...
    async getBackpackItems() {
        try {
            ...
            console.error('数据适配器：获取背包物品失败', error);
    ...
    async addBackpackItem(itemData) {
        try {
            ...
            console.error('数据适配器：添加背包物品失败', error);
            throw error;
        }
    }
    ...
    async useBackpackItem(itemId, quantity = 1) {
        try {
            ...
            console.error('数据适配器：使用背包物品失败', error);
            throw error;
        }
    }
    ...
    async createOrder(orderData) {
        try {
            ...
            console.error('数据适配器：创建订单失败', error);
            throw error;
        }
    }
    ...
    async getUserOrders() {
        try {
            ...
            console.error('数据适配器：获取用户订单失败', error);
            return { success: false, error: error.message };
        }
    }
