...
class UserSystem {
    ...
    async checkLoginStatus() {
        ...
        try {
            ...
            if (!this.currentUser || !this.currentUser.name) {
                console.log('🌐 用户系统：用户数据格式错误，跳转到登录页面');
                this.redirectToLogin();
                return false;
            }
            ...
            if (this.forceCloudMode && window.dataAdapter) {
                try {
                    const cloudUser = await window.dataAdapter.getUserInfo();
                    if (cloudUser && cloudUser.id === this.currentUser.id) {
                        // 云端验证成功，更新本地数据
                        this.currentUser = cloudUser;
                        localStorage.setItem('currentUser', JSON.stringify(cloudUser));
                        console.log('🌐 用户系统：云端验证成功');
                    } else {
                        console.log('🌐 用户系统：云端验证失败，跳转到登录页面');
                        this.redirectToLogin();
                        return false;
                    }
                } catch (cloudError) {
                    console.error('🌐 用户系统：云端验证失败，降级到本地', cloudError);
                    // 降级到本地验证
                }
            }
            ...
            if (this.forceCloudMode && window.dataAdapter) {
                try {
                    ...
                    for (const order of this.orders) {
                        await window.dataAdapter.createOrder(order);
                    }
                    console.log('🌐 用户系统：订单数据已同步到云端');
                } catch (cloudError) {
                    console.error('❌ 用户系统：云端同步订单失败', cloudError);
                    // 降级到本地存储
                    localStorage.setItem('userOrders', JSON.stringify(this.orders));
                    console.log('💾 用户系统：订单数据已降级保存到本地');
                }
            } else {
                // 本地备份
                localStorage.setItem('userOrders', JSON.stringify(this.orders));
                console.log('💾 用户系统：订单数据已保存到本地');
            }
        } catch (error) {
    ...
    async createOrder(orderData) {
        try {
            ...
            if (window.dataAdapter) {
                try {
                    await window.dataAdapter.createOrder(order);
                    console.log('🌐 用户系统：订单已同步到云端');
                } catch (error) {
                    console.error('❌ 用户系统：同步订单到云端失败', error);
                }
            }
    ...
    async updateOrderStatus(orderId, status, note = '') {
        try {
            ...
            if (order) {
                ...
                if (window.dataAdapter) {
                    try {
                        await window.dataAdapter.updateOrderStatus(orderId, { status });
                        console.log('🌐 用户系统：订单状态已同步到云端');
                    } catch (error) {
                        console.error('❌ 用户系统：同步订单状态到云端失败', error);
                    }
                }
    ...
    async logout() {
        try {
            ...
            this.showMessage('error', '退出登录失败', error.message);
        }
    }
