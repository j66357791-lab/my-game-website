// backend/websocket/gameWebSocket.js - 增强容错版 + 集成智能管控 (修复配置读取一致性问题)
...
class GameWebSocket {
  ...
  initialize(server) {
    ...
    this.startGameLoop();
    this.startHeartbeat();
    console.log('🎮 神秘卡牌WebSocket服务器启动成功 (集成智能管控版 - 配置一致修复)');
  }
  ...
  // 🔧 删除：不再需要本地的 checkAutoBalance 方法，使用 adminController.checkAutoBalanceControl()
  // 这样确保配置读取的一致性
