// backend/websocket/gameWebSocket.js - 增强容错版 + 集成智能管控 (修复配置读取一致性问题 + 修复重复键报错)
...
// 🔧 引入 adminController 以使用其智能防亏检查（确保配置一致）
...
class GameWebSocket {
  ...
  initialize(server) {
    ...
    this.startGameLoop();
    this.startHeartbeat();
    console.log('🎮 神秘卡牌WebSocket服务器启动成功 (集成智能管控版 - 配置一致修复 + 重复键错误修复)');
  }
  ...
  // 🔧 修改：准备新的一轮 (集成智能防亏 + 管理员控制 - 修复配置读取一致性问题)
  async prepareNewRound() {
    ...
    // ==================== 1. 智能防亏检查 (最高优先级 - 使用 adminController 确保配置一致) ====================
    const autoResult = await adminController.checkAutoBalanceControl();
    if (autoResult) {
  ...
  async settleBets() {
    ...
    try {
      ...
      try {
        ...
        console.log(`📝 本局历史记录已保存至 DB (Round ${this.gameState.roundNumber})`);
      } catch (historyErr) {
        // 🔥 修复：处理重复键错误（通常是多实例竞争导致的，忽略即可）
        if (historyErr.code === 11000) {
          console.warn(`⚠️ 轮次 ${this.gameState.roundNumber} 记录已存在（可能是其他实例已保存），跳过保存。`);
        } else {
