// backend/websocket/gameWebSocket.js - 最终修复版 (修正Transaction类型错误)
...
class GameWebSocket {
  ...
  initialize(server) {
    ...
    // 启动游戏循环
    this.startGameLoop();
    console.log('🎮 神秘卡牌WebSocket服务器启动成功 (最终修复版)');
  }
  ...
  async handleBet(userId, { general, amount }) {
    ...
    try {
      ...
      const transaction = new Transaction({
        userId,
        type: 'bet', // 🔧 修正：使用 'bet' 而不是 'game_bet'
        amount: -amount,
        balance: user.points,
      ...
      for (const [userId, userBets] of this.gameState.bets) {
        ...
        try {
          ...
          if (totalWinAmount > 0) {
            ...
            const transaction = new Transaction({
              userId,
              type: 'win', // 🔧 修正：使用 'win' 而不是 'game_win'
