// backend/websocket/gameWebSocket.js - 增强容错版 (解决心跳超时断连)
...
class GameWebSocket {
    ...
    try {
      ...
      switch(type) {
        ...
        case 'GET_HISTORY':
          ...
          // 限流：3秒内只处理一次（从1秒改为3秒，进一步减轻数据库压力）
  ...
  async sendHistory(userId) {
    try {
      // 🔧 优化：只查询最新 10 条历史记录，而不是 20 条（减少数据传输量）
  ...
  // 🔧 核心修复：增强心跳容错
  startHeartbeat() {
    const heartbeatInterval = 20000; // 每 20 秒发送一次 PING
    const disconnectTimeout = 120000; // 2 分钟没收到 PONG 才断开（从 60 秒延长到 120 秒）
