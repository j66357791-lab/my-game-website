// backend/websocket/gameWebSocket.js - 根本修复版
...
class GameWebSocket {
  ...
  initialize(server) {
    ...
    // 启动游戏循环
    this.startGameLoop();
    console.log('🎮 神秘卡牌WebSocket服务器启动成功 (根本修复版)');
  }
  ...
  async handleBet(userId, { general, amount }) {
    ...
    try {
      ...
      const transaction = new Transaction({
        userId,
        type: 'mystery_bet', // 🔧 根本修复：使用新增的 'mystery_bet' 类型
        amount: -amount,
        balance: user.points,
      ...
      for (const [userId, userBets] of this.gameState.bets) {
        ...
        try {
          ...
          if (totalWinAmount > 0) {
            ...
            const transaction = new Transaction({
              userId,
              type: 'mystery_win', // 🔧 根本修复：使用新增的 'mystery_win' 类型
