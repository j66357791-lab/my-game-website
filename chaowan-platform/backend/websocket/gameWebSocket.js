// backend/websocket/gameWebSocket.js - 增强容错版 (解决心跳超时断连) + 集成控制台
...
class GameWebSocket {
  ...
  initialize(server) {
    ...
    this.startGameLoop();
    this.startHeartbeat();
    console.log('🎮 神秘卡牌WebSocket服务器启动成功 (增强容错版)');
  }
  ...
  handleConnection(ws, req) {
    ...
    try {
      ...
      ws.userId = userId;
      ws.isAlive = true;
      ws.lastPongTime = Date.now(); // 记录最后一次PONG时间
  ...
  async prepareNewRound() {
    ...
    try {
      ...
      if (config.mode === 'FIXED') {
        ...
        lordValue = Math.floor(Math.random() * 10) + 1;
      }
    } catch (err) {
      console.error('❌ 读取控制配置失败，使用随机:', err);
      lordValue = Math.floor(Math.random() * 10) + 1;
    }

    const generateGeneral = () => Math.floor(Math.random() * 10) + 1;
    ...
    console.log('🎴 卡牌已生成:', this.gameState.roundNumber, '领主:', lordValue);
  }
    ...
    try {
      for (const [userId, userBets] of this.gameState.bets) {
        ...
        try {
          ...
          const verifyUser = await User.findById(userId);
          console.log(`[DEBUG] 用户 ${userId} 事务提交后 DB 余额: ${verifyUser.points}`);
      ...
      try {
        ...
        await MysteryCardGame.create({
          ...
          results: { east: 'draw', south: 'draw', west: 'draw', north: 'draw' }, // 简化占位
