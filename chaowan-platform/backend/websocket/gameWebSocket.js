...
class GameWebSocket {
    ...
    try {
      ...
      switch(type) {
        ...
        case 'GET_HISTORY':
          ...
          break;
        case 'GET_GAME_HISTORY': // 🔧 新增：走势图数据请求
  ...
  async handleBet(userId, { general, amount }) {
    ...
    try {
      ...
      // 🔧 新增：用户个人下注限制 (单局累计下注不能超过自身积分的10%)
      const currentUserBets = this.gameState.bets.get(userId) || [];
      const currentTotalBet = currentUserBets.reduce((sum, bet) => sum + bet.amount, 0);
      const userMaxBetLimit = Math.floor(user.points * 0.1); // 10% 限制

      if (currentTotalBet + amount > userMaxBetLimit) {
        throw new Error(`本局累计下注已达个人上限 (${currentTotalBet}/${userMaxBetLimit})`);
      }
  ...
  async sendGameHistory(userId) {
    try {
      const history = await MysteryCardGame.find({})
        .sort({ roundNumber: -1 })
        .limit(10)
        .sort({ roundNumber: 1 }); // 升序返回，方便前端绘图
