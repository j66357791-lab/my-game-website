...
class GameWebSocket {
  ...
  initialize(server) {
    ...
    this.startHeartbeat();
    console.log('🎮 领主模式 WebSocket服务器启动 (完整安全版)');
  }
  ...
  async handleBet(userId, { general, amount }) {
    ...
    try {
      ...
      const userMaxBetLimit = Math.floor(user.points * 0.1); // 用户自身积分的10%
  ...
  // 发送走势图数据 (仅查询数据库，不包含当前局)
  async sendGameHistory(userId) {
    try {
      // 只查询数据库中已保存的历史记录 (最近10期)
      const history = await MysteryCardGame.find({})
        .sort({ roundNumber: -1 })
        .limit(10)
        .sort({ roundNumber: 1 }); // 重新排序为升序，方便前端绘图
