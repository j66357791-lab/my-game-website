// backend/websocket/gameWebSocket.js - 终极完整版 (含心跳检测 + 新规则结算)
...
class GameWebSocket {
  ...
  initialize(server) {
    ...
    console.log('🎮 神秘卡牌WebSocket服务器启动成功 (终极完整版)');
  }
  ...
  async handleBet(userId, { general, amount }) {
    ...
    try {
      ...
      // 2. 校验：余额是否足够 (注意：由于有惩罚倍率，这里只需要校验本金是否足够)
      ...
      for (const [userId, userBets] of this.gameState.bets) {
        ...
        try {
          ...
          for (const bet of userBets) {
            ...
            let result = 'lose';
            let multiplier = 0;
            let extraLoss = 0; // 额外扣除的积分

            // 游戏规则：战将 > 领主 则赢，反之则输
            // 规则修正：0点也是1倍，胜利按战将点数倍，失败按领主点数倍扣除
            ...
            if (generalStar > lordStar) {
              ...
              multiplier = -lossMultiplier; // 负数表示损失
              
              // 计算额外扣除金额：本金已扣除，现在扣除 (倍率-1) * 本金
              extraLoss = amount * (lossMultiplier - 1);
              
              if (extraLoss > 0) {
                user.points -= extraLoss; // 扣除额外积分
                hasLoss = true;
              }
            }
          ...
          if (totalWinAmount > 0) {
            ...
            // 只有输的情况，points 已经在循环中扣除了 extraLoss，直接保存
            const oldPoints = user.points; // 注意这里 points 已经是扣除后的了
            ...
            const transaction = new Transaction({
              ...
              metadata: { 
                round: this.gameState.roundNumber, 
                results: roundResults,
                oldBalance: oldPoints + totalLoss // 扣除前的余额
              }
            });
            await transaction.save({ session });
  ...
  startHeartbeat() {
    ...
    const checkAlive = () => {
      // 使用 wss.clients (WebSocket库自带的 Set) 或 this.clients
