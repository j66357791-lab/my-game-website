...
dollSchema.methods.recycle = function() {
  ...
  // 🔧 修复：使用正确公式 0.05积分 × 娃娃剩余存活天数
  const recycleReward = Math.floor(0.05 * this.remainingDays);
  
  console.log(`🔄 娃娃回收计算:`);
  console.log(`  - 娃娃名称: ${this.name}`);
  console.log(`  - 剩余天数: ${this.remainingDays}`);
  console.log(`  - 回收公式: 0.05 × ${this.remainingDays} = ${recycleReward} 积分`);
