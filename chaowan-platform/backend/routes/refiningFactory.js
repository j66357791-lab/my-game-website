// backend/routes/refiningFactory.js - 数据一致性修复版本
...
// 🔧 关键修复：确保盲盒活动数据存在
const ensureBlindBoxActivity = async (userId) => {
  let blindBoxActivity = await BlindBoxActivity.findOne({ userId });
  if (!blindBoxActivity) {
    blindBoxActivity = new BlindBoxActivity({
      userId,
      collectedChars: [],
      totalDraws: 0
    });
    await blindBoxActivity.save();
    console.log('✅ 创建新的盲盒活动数据:', userId);
  }
...
router.get('/data', authMiddleware, async (req, res) => {
  try {
    ...
    // 🔧 关键修复：确保盲盒活动数据存在
    const blindBoxActivity = await ensureBlindBoxActivity(req.user._id);
    ...
    if (factory.status === 'active' && factory.refiningStartTime) {
      ...
      if (remainingTime === 0) {
        ...
        factory.refinedPoints = factory.inputChars.length * 10;
        await factory.save();
    ...
    res.json({
      ...
      data: {
        ...factory.toObject(),
        remainingTime,
        refiningSpeed: speedSeconds,
        speedDisplay,
        // 🔧 新增：返回盲盒背包数据（用于前端显示）
        blindBoxChars: blindBoxActivity.collectedChars || [],
        blindBoxCharsCount: (blindBoxActivity.collectedChars || []).length
      }
    });
  } catch (error) {
  ...
  try {
    ...
    for (const [char, count] of Object.entries(inputCharCounts)) {
      if ((backpackCharCounts[char] || 0) < count) {
        ...
        return res.status(400).json({ 
          success: false, 
          message: `字符"${char}"不足，需要${count}个，但背包中只有${backpackCharCounts[char] || 0}个` 
        });
      }
    }
    ...
    chars.forEach(char => {
      ...
      if (index > -1) {
        ...
        // 如果找不到字符，说明数据不一致
        await session.abortTransaction();
        throw new Error(`背包中找不到字符"${char}"，数据可能不一致`);
      }
    });

    // 保存盲盒活动数据
    await blindBoxActivity.save({ session });
    ...
    console.log(`📊 背包剩余字符: ${blindBoxActivity.collectedChars.length}个`);
...
router.post('/withdraw', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    console.log(`📊 背包新增字符: ${withdrawnChars.length}个，总计: ${blindBoxActivity.collectedChars.length}个`);
