...
// ==================== 🔧 临时修复：批量加密老用户密码 ====================
app.post('/api/auth/migrate-passwords', adminMiddleware, async (req, res) => {
  try {
    console.log('🔧 开始批量迁移用户密码...');
    ...
    for (const user of users) {
      // 检查密码是否已经是加密的（bcrypt 加密的密码通常以 $2a$ 或 $2b$ 开头）
      if (user.password && !user.password.startsWith('$2')) {
        console.log(`🔄 加密用户: ${user.username} (${user.email})`);
        
        // 重新设置密码会触发 User 模型的 pre('save') 钩子，自动加密
    ...
    console.log(`✅ 密码迁移完成！已加密: ${migratedCount} 个，已跳过: ${skippedCount} 个`);
