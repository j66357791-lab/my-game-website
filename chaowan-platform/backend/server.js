// backend/server.js - 神秘卡牌完整修复版本
const express = require('express');
const http = require('http');  // 🔧 新增：支持WebSocket
...
try {
  ...
  console.log('✅ Doll模型导入成功');
} catch (error) {
  console.error('❌ Doll模型导入失败:', error);
  process.exit(1); // 如果 Doll 模型导入失败，直接退出
}
...
const mysteryCardRoutes = require('./routes/mysteryCard');  // 🔧 新增
...
app.use('/api/refining-factory', refiningFactoryRoutes); // 🔧 修复：使用连字符路径

// 🔧 新增：使用神秘卡牌路由
app.use('/api/mystery-card', mysteryCardRoutes);

// 🔧 新增：使用VIP卡和Boss路由 - 修复路径
app.use('/api/vip-cards', vipCardRoutes); // 修复：从 vip-card 改为 vip-cards
...
mongoose.connection.once('open', () => {
  initializeTestUser();
  initializeBoss(); // 添加Boss初始化
  initializeMysteryCard(); // 添加神秘卡牌初始化
});
...
const authMiddleware = (req, res, next) => {
  try {
    ...
    User.findById(decoded.userId).select('-password').then(user => {
      ...
      // 🔧 设置完整的用户对象，包含 _id 字段
      req.user = user; // 这样 req.user._id 就存在了
      console.log('✅ 认证成功，用户ID:', req.user._id);
      next();
    }).catch(error => {
      ...
      res.json({ 
        ...
        data: { 
          user: { 
            ...
            starcoin: user.starcoin // 添加星源币字段
          }, 
          token 
        } 
      });
...
// 🔧 新增：获取用户现金交易记录API
app.get('/api/transactions/cash', authMiddleware, async (req, res) => {
  try {
...
app.put('/api/admin/users/:userId', adminAuth, async (req, res) => {
  try {
    ...
    if (starcoin !== undefined) updateData.starcoin = starcoin; // 添加星源币更新
...
app.put('/api/admin/withdrawals/:id/process', adminAuth, async (req, res) => {
  try {
    ...
    const adminId = req.adminUser._id; // 🔧 修复：使用 adminUser._id
...
app.use('*', (req, res) => {
  res.status(404).json({
    ...
    availableEndpoints: [
      ...
      'POST /api/dolls/purchase',
      'GET /api/dolls/my',
      'POST /api/dolls/:dollId/recycle',
      ...
      'GET /api/blindBox/activity',
      'POST /api/blindBox/single-draw',
      'POST /api/blindBox/ten-draw',
      'POST /api/blindBox/exchange',
      'GET /api/blindBox/exchange-history',
      'GET /api/refining-factory',
      'POST /api/refining-factory/input',
      'POST /api/refining-factory/withdraw',
      'POST /api/refining-factory/claim',
      'GET /api/refining-factory/history',
      // 添加VIP和Boss端点
      'GET /api/vip-cards/status',
      'POST /api/vip-cards/purchase',
      'POST /api/vip-cards/claim-daily-starcoin',
      'GET /api/boss/status',
      ...
      // 🔧 新增：神秘卡牌端点
      'GET /api/mystery-card/history',
      'GET /api/mystery-card/user-history',
      'WebSocket: wss://tianchuang.onrender.com/?token=your_jwt_token'
    ]
  });
});
