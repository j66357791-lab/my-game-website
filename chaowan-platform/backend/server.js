...
const cors = require('cors'); // 🔥 使用 cors 库
...
// 🔥 修复：根据上传的文件名 vip.js，这里引入路径改为 ./routes/vip
// (如果你将文件改名为 vipCard.js，请改回 require('./routes/vipCard'))
const vipCardRoutes = require('./routes/vip'); 
...
app.use(cors({
  origin: [
    'https://chaowan-frontend.onrender.com', // 你的前端域名
...
app.use('/api/vip-cards', vipCardRoutes); // 挂载 VIP 路由
...
// ==================== 🔧 修复版：强制批量加密老用户密码 ====================
app.post('/api/auth/migrate-passwords-force', adminMiddleware, async (req, res) => {
  try {
...
app.get('/api/admin/mystery-card/config', adminMiddleware, getMysteryCardConfig);
app.post('/api/admin/mystery-card/config', adminMiddleware, updateMysteryCardConfig);
app.get('/api/admin/mystery-card/stats', adminMiddleware, getMysteryCardStats);
...
app.put('/api/admin/users/:userId/password', adminMiddleware, async (req, res) => {
  try {
    const { userId } = req.params;
...
app.put('/api/admin/users/:userId/toggle-status', adminMiddleware, async (req, res) => {
  try {
    const { userId } = req.params;
    const user = await User.findById(userId);
    if (!user) {
...
app.post('/api/admin/points/adjust', adminMiddleware, async (req, res) => {
  try {
...
app.post('/api/admin/cash/adjust', adminMiddleware, async (req, res) => {
  try {
    const { userId, amount, description } = req.body;
    const user = await User.findById(userId);
    if (!user) {
...
app.put('/api/admin/withdrawals/:id/process', adminMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
...
app.post('/api/admin/withdrawals/batch-process', adminMiddleware, async (req, res) => {
  try {
