...
app.get('/api/health', (req, res) => {
  res.json({
    ...
    gameModels: {
      GameSession: GameSession ? 'loaded' : 'not loaded',
      Bet: Bet ? 'loaded' : 'not loaded'
    }
  });
});
...
app.get('/api/admin/dashboard', adminAuth, async (req, res) => {
  try {
    ...
    const activeGames = await GameSession.countDocuments({ status: { $ne: 'finished' } });
...
app.get('/', (req, res) => {
  res.json({ 
    ...
    apis: [
      ...
      'GET /api/game/current - 获取当前游戏',
      'POST /api/game/bet - 下注',
...
// 🔧 关键修复：删除重复的游戏状态管理，只保留 routes/game.js 中的
mongoose.connection.once('open', () => {
  initializeTestUser();
  
  // 🔧 同步游戏模型
  console.log('🔄 同步游戏模型...');
  // GameSession.sync && GameSession.sync({ alter: true });  // 🔧 删除这些，MongoDB 不需要 sync
  // Bet.sync && Bet.sync({ alter: true });
  
  // 🔧 删除重复的游戏管理启动代码，因为 routes/game.js 中已经有了
  console.log('✅ 数据库连接完成，游戏路由已启动');
});
...
app.listen(PORT, () => {
  ...
  console.log(`🎲 图标大乱斗游戏已启用`);
  console.log(`🔍 Doll模型状态:`, Doll ? '已加载' : '未加载');
  console.log(`🎮 游戏模型状态:`, (GameSession && Bet) ? '已加载' : '未加载');
});
