...
// 🔧 修复：购买娃娃API - 添加详细错误处理
app.post('/api/dolls/purchase', authMiddleware, async (req, res) => {
  try {
    const { dollId, dollLevel } = req.body;
    const userId = req.user.id;

    console.log(`🧸 开始购买娃娃: userId=${userId}, dollLevel=${dollLevel}`);
    console.log(`🔍 Doll模型状态:`, Doll ? '已加载' : '未加载');

    // 🔧 关键修复：检查 Doll 模型
    if (!Doll) {
      console.error('❌ Doll模型未加载');
      return res.status(500).json({
        success: false,
        message: '娃娃模型未加载，请联系管理员'
      });
    }
    ...
    let dollConfig;
    try {
      dollConfig = Doll.getDollConfig(dollLevel);
      console.log(`🔍 娃娃配置:`, dollConfig);
    } catch (configError) {
      console.error('❌ 获取娃娃配置失败:', configError);
      return res.status(500).json({
        success: false,
        message: '获取娃娃配置失败: ' + configError.message
      });
    }
    
    if (!dollConfig) {
      console.log(`❌ 娃娃配置不存在: dollLevel=${dollLevel}`);
      return res.status(400).json({
        success: false,
        message: '娃娃配置不存在'
      });
    ...
    if (user.points < dollConfig.purchasePrice) {
      console.log(`❌ 积分不足: userPoints=${user.points}, required=${dollConfig.purchasePrice}`);
      return res.status(400).json({
        success: false,
        message: '积分不足'
      });
    ...
    try {
      ...
      console.log(`🔧 娃娃对象创建完成，准备保存...`);
      await doll.save();
      console.log(`✅ 娃娃创建成功: dollId=${doll._id}`);
    } catch (dollError) {
      console.error('❌ 创建娃娃失败:', dollError);
      console.error('❌ 详细错误:', dollError.message);
      console.error('❌ 错误堆栈:', dollError.stack);
      return res.status(500).json({
        success: false,
        message: '创建娃娃失败: ' + dollError.message
      });
    }
    ...
    try {
      console.log(`🔧 开始扣除积分...`);
      await user.save();
      console.log(`✅ 积分扣除成功: oldPoints=${oldPoints}, newPoints=${user.points}`);
    } catch (userError) {
      console.error('❌ 扣除积分失败:', userError);
      // 如果扣除积分失败，删除已创建的娃娃
      try {
        await Doll.findByIdAndDelete(doll._id);
        console.log(`🔧 已回滚娃娃创建: dollId=${doll._id}`);
      } catch (rollbackError) {
        console.error('❌ 回滚娃娃创建失败:', rollbackError);
      }
      return res.status(500).json({
        success: false,
        message: '扣除积分失败: ' + userError.message
      });
    }
    ...
    try {
      ...
      const transaction = new Transaction({
        ...
        relatedId: doll._id,
        metadata: {
          dollLevel: dollConfig.level,
          dollName: dollConfig.name
        }
      });
      await transaction.save();
      console.log(`✅ 交易记录创建成功: transactionId=${transaction._id}`);
    } catch (transactionError) {
      console.error('❌ 创建交易记录失败:', transactionError);
      // 交易记录失败不影响主要流程
    }

    console.log(`🎉 娃娃购买完成: ${user.username}, ${dollConfig.name}, 消耗${dollConfig.purchasePrice}积分`);
...
// 🔧 新增：获取用户娃娃列表API
app.get('/api/dolls/my', authMiddleware, async (req, res) => {
  try {
    const userId = req.user.id;
...
// 🔧 新增：回收娃娃API
app.post('/api/dolls/:dollId/recycle', authMiddleware, async (req, res) => {
  try {
    ...
    console.log(`♻️ 用户回收娃娃: ${user.username}, ${doll.name}, 获得${recycleResult.reward}积分`);
