// backend/server.js - 完整修复版 (已集成星源币市场)
...
const immortalRoutes = require('./routes/immortal');
const alchemistRoutes = require('./routes/alchemist');
...
const MarketWS = require('./websocket/marketWebSocket'); // ✅ 新增：市场WebSocket
const starcoinRoutes = require('./routes/starcoin');       // ✅ 新增：市场路由
...
// ✅ 新增：初始化星源币市场 WebSocket (独立路径，互不干扰)
MarketWS.initialize(server);

// 🔥 CORS配置 - 彻底解决跨域问题
app.use(cors({
  origin: [
    'https://chaowan-frontend.onrender.com', // 旧的前端域名
    'https://tianchuang.zeabur.app', // ✅ 新增：你的域名
    'https://tianchang.zeabur.app',        // 你的后端域名
    'http://localhost:3000',
    'http://localhost:5173'
  ],
...
app.use('/api/alchemist', alchemistRoutes);

// ✅ 新增：挂载星源币市场路由
app.use('/api/starcoin', starcoinRoutes);
...
app.get('/', (req, res) => {
  res.json({ 
    ...
    apis: [
      ...
      '📈 /api/starcoin/order - 星源币下单',
      '📈 /api/starcoin/klines - 获取K线',
      '📈 /api/starcoin/depth - 获取盘口深度',
      '🔌 WebSocket连接: wss://https://tianchang.zeabur.app//?token=your_jwt_token',
      '📈 Market WebSocket: wss://https://tianchang.zeabur.app/market?token=your_jwt_token'
    ]
  });
});
...
server.listen(PORT, () => {
  ...
  console.log(`📈 星源币市场WebSocket已启动`); // ✅ 新增日志
