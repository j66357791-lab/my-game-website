...
const Withdrawal = require('./models/Withdrawal'); // 🔧 新增
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    const newUser = new User({
      ...
      cashBalance: isAdminEmail ? 1000 : 0 // 🔧 新增初始余额
    });
    ...
    res.status(201).json({
      ...
      data: {
        user: {
          ...
          role: newUser.role,
          cashBalance: newUser.cashBalance // 🔧 新增
        },
        token
      }
    });
      ...
      res.json({ 
        ...
        data: { 
          user: { 
            ...
            role: user.role,
            cashBalance: user.cashBalance // 🔧 新增
          }, 
          token 
        } 
      });
...
app.delete('/api/admin/users/:userId', adminAuth, async (req, res) => {
  try {
    ...
    await Withdrawal.deleteMany({ userId }); // 🔧 新增
