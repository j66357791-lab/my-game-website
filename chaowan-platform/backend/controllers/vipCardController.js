// backend/controllers/vipCardController.js - 增强日志版
...
exports.getVipStatus = async (req, res) => {
  try {
    const userId = req.user.id; // 或者 req.user._id
...
exports.claimDailyStarcoin = async (req, res) => {
  try {
    console.log(`💎 [VIP] 用户 ${req.user.id} 尝试领取星源币...`);
    const userId = req.user.id; // 建议改为 req.user._id
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const tomorrow = new Date(today);
    tomorrow.setDate(tomorrow.getDate() + 1);
    ...
    if (existingTransaction) {
      console.log(`⚠️ [VIP] 用户 ${userId} 今日已领取`);
    ...
    if (vipCards.length === 0) {
      console.log(`❌ [VIP] 用户 ${userId} 没有有效VIP卡`);
    ...
    console.log(`✅ [VIP] 用户 ${userId} 应获得 ${totalStarcoin} 星源币`);

    // 增加星源币
    const user = await User.findById(userId);
    const oldBalance = user.starcoin || 0;
    user.starcoin += totalStarcoin;
    
    await user.save(); // 🔥 关键：保存
    console.log(`💰 [VIP] 数据库已更新: ${oldBalance} -> ${user.starcoin}`);
    ...
    // 🔥 返回结果带上 newBalance，方便前端更新
    res.json({ 
      success: true, 
      message: `领取成功，获得${totalStarcoin}星源币`, 
      data: { 
        totalStarcoin,
        newBalance: user.starcoin 
      } 
    });
  } catch (error) {
    console.error(`❌ [VIP] 领取失败:`, error);
