...
const getMyDoll = async (req, res) => {
  try {
    ...
    let totalAntiCrit = attrs.antiCritRate || 0;
    let totalAntiDodge = attrs.antiDodgeRate || 0;
    ...
    Object.keys(doll.equipmentSlots).forEach(slot => {
      ...
      if (equip) {
        ...
        totalAtk += (stats.attack || 0);
        totalHp += (stats.health || 0);
        totalDef += (stats.defense || 0);
        totalCrit += (stats.critRate || 0);
        totalDodge += (stats.dodgeRate || 0);
        totalAntiCrit += (stats.antiCritRate || 0);
        totalAntiDodge += (stats.antiDodgeRate || 0);
    ...
    dollObj.totalAttributes = {
      attack: totalAtk, health: totalHp, defense: totalDef, critRate: totalCrit,
      dodgeRate: totalDodge, antiCritRate: totalAntiCrit, antiDodgeRate: totalAntiDodge,
      speed: attrs.speed || 0, totalSpiritRate: totalSpiritRate
    };
...
const challengeDungeon = async (req, res) => {
  try {
    ...
    let currentPlayerHp = doll.totalAttributes.health;
    const playerAtk = doll.totalAttributes.attack;
    const playerDef = doll.totalAttributes.defense;
    const playerSpd = doll.totalAttributes.speed || 0;
    
    // 简单战斗判定
    if (currentEnemyHp > 0) {
       const turnsToWin = Math.ceil(enemy.hp / Math.max(1, playerAtk));
       const totalDmg = Math.max(0, enemy.attack - playerDef) * turnsToWin;
       isWin = currentPlayerHp > totalDmg;
    }
    ...
    res.json({ 
      ...
      data: { doll, rewards, userStarcoin: user.starcoin } 
    });
  } catch (error) {
...
const unequipItem = async (req, res) => {
  try {
    ...
    await doll.save();
    res.json({ success: true, message: '卸下成功', data: { doll } });
  } catch (error) {
...
const refineItem = async (req, res) => {
  try {
    ...
    if (isSuccess) {
      ...
      user.refineStones -= 1;
      res.json({ success: true, message: `强化成功！当前等级 +${equip.level}`, data: { equip } });
    } else {
      if (equip.level > 0) {
        equip.level -= 1;
        await material.deleteOne();
        user.refineStones -= 1;
        res.json({ success: false, message: `强化失败！装备降级至 ${equip.level}`, data: { equip } });
      } else {
        await ImmortalEquipment.deleteOne({ _id: equipId });
        await material.deleteOne();
        user.refineStones -= 1;
        res.json({ success: false, message: '强化失败！装备已损毁', data: { equip: null } });
      }
    }
    await equip.save();
    await user.save();
