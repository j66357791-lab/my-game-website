...
const getMyDoll = async (req, res) => {
  try {
    ...
    let cp = 0;
    let totalAtk = attrs.attack;
    let totalHp = attrs.health;
    let totalDef = attrs.defense;
    let totalCrit = attrs.critRate;
    let totalDodge = attrs.dodgeRate;
...
const challengeDungeon = async (req, res) => {
  try {
    ...
    const enemy = { name: '狗熊', hp: 50, attack: 5, defense: 0 };
    ...
    if (isWin) {
      ...
      await user.save();
      await doll.save();
      res.json({ success: true, message: `挑战成功！`, data: { doll, rewards, userStarcoin: user.starcoin } });
    } else {
      await user.save();
