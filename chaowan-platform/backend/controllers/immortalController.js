...
const challengeDungeon = async (req, res) => {
  try {
    ...
    // 获取玩家属性 (目前只取娃娃基础属性，后续计算装备后需要加上装备属性)
    const playerAtk = Math.max(1, doll.baseAttributes.attack); // 至少1点攻击
    ...
    let rewards = {
      spirit: 0,
      equipments: [],
      stones: { spiritStone: 0, refineStone: 0 }
    };

    if (isWin) {
      // 5. 胜利结算
      
      // 5.1 固定掉落灵气 (1000-5000)
      const spiritGain = Math.floor(Math.random() * 4000) + 1000;
      doll.spiritualEnergy += spiritGain;
      rewards.spirit = spiritGain;

      // 5.2 随机掉落装备 (0-3件)
      const equipCount = Math.floor(Math.random() * 4); // 0, 1, 2, 3
      ...
      // 强化石 10% (0.1)
      if (Math.random() < 0.1) {
        rewards.stones.refineStone += 1;
      }
      // 灵气石 5% (0.05)
      if (Math.random() < 0.05) {
        rewards.stones.spiritStone += 1;
      }
      ...
      res.json({
        success: true,
        message: `挑战成功！击败了 ${enemy.name}`,
        data: {
          doll,
          rewards,
          userStarcoin: user.starcoin // 返回最新余额
        }
      });
      ...
      res.json({
        success: false, // 业务失败
        message: `挑战失败！你被 ${enemy.name} 击败了。请提升实力再来！`,
        data: {
          doll,
