    ...
    res.status(201).json({
      ...
      data: {
        user: {
          ...
          starcoin: newUser.starcoin.toFixed(2), // 星源币保留两位小数
    ...
    res.status(200).json({
      ...
      data: {
        user: {
          ...
          level: user.level,
          points: user.points,
          starcoin: user.starcoin.toFixed(2), // 星源币保留两位小数
    ...
    res.status(200).json({
      ...
      data: {
        user: {
          ...
          level: user.level,
          points: user.points,
          starcoin: parseFloat(user.starcoin).toFixed(2), // 星源币保留两位小数
          cashBalance: user.cashBalance,
          experience: user.experience,
          role: user.role
        }
...
const getGameStats = async (req, res) => {
  try {
    ...
    const betStats = await Transaction.aggregate([
      {
        $match: {
          userId: new mongoose.Types.ObjectId(userId), // ✅ 修正：使用 new mongoose.Types.ObjectId
          type: { $in: betTypes },
          currency: 'points',
    ...
    const winStats = await Transaction.aggregate([
      {
        $match: {
          userId: new mongoose.Types.ObjectId(userId), // ✅ 修正
          type: { $in: winTypes },
          currency: 'points',
          createdAt: { $gte: startDate }
        }
