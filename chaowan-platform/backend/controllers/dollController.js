...
// 获取用户娃娃列表
const getUserDolls = async (req, res) => {
  try {
...
// 购买娃娃（使用starcoin）
const purchaseDoll = async (req, res) => {
  try {
...
// 派遣娃娃出战
const deployDoll = async (req, res) => {
  try {
...
// 召回娃娃
const recallDoll = async (req, res) => {
  try {
    const { dollId } = req.body;
    const userId = req.user.id;
...
// 批量购买娃娃
const purchaseDolls = async (req, res) => {
  try {
...
// 领取今日收益
const claimDailyEarnings = async (req, res) => {
  try {
    const userId = req.user.id;
...
// 回收娃娃
const recycleDoll = async (req, res) => {
  try {
