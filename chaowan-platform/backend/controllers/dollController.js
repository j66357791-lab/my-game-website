// backend/controllers/dollController.js - 完全修复版（解决收益领取问题）
...
exports.getUserDolls = async (req, res) => {
  try {
    ...
    const processedDolls = dolls.map(doll => ({
      ...
      rarity: doll.rarity,
      purchasePrice: parseFloat(doll.purchasePrice || 0).toFixed(2),
      productionPerDay: parseFloat(doll.productionPerDay || 0).toFixed(2),
...
exports.purchaseDoll = async (req, res) => {
  try {
    ...
    const dollConfig = dollConfigs[parsedDollLevel];
    if (!dollConfig) {
      return res.status(400).json({
        success: false,
        message: '娃娃配置不存在'
...
exports.claimDailyEarnings = async (req, res) => {
  try {
    ...
    // 🔧 关键修复：手动计算每个娃娃的今日产出，不依赖produceDaily方法
    for (const doll of dolls) {
      // 🔧 直接使用娃娃的productionPerDay属性
      const dailyEarning = parseFloat(doll.productionPerDay || 0);
      
      console.log(`🧸 娃娃 ${doll.name} (${doll.emoji}) 今日产出: ${dailyEarning}`);
      
      if (dailyEarning > 0) {
        totalEarnings += dailyEarning;
        
        // 🔧 更新娃娃的总产出
        doll.totalProduced = parseFloat((doll.totalProduced || 0) + dailyEarning).toFixed(2);
...
exports.recycleDoll = async (req, res) => {
  try {
    ...
    // 🔧 手动计算回收价值
    const recycleReward = parseFloat((doll.remainingDays || 0) * 0.5).toFixed(2);
    let recycleExperience = 0;
    
    // 二级娃娃额外获得经验
    if (doll.level === 2) {
      recycleExperience = parseFloat((doll.remainingDays || 0) * 2).toFixed(2);
    }
    
    // 标记为已回收
    doll.isRecycled = true;
    doll.recycledAt = new Date();
    await doll.save();
    ...
    user.points = parseFloat(oldPoints + recycleReward).toFixed(2);
    user.experience = parseFloat(oldExperience + recycleExperience).toFixed(2);
    await user.save();
    ...
    await new Transaction({
      userId: userId,
      type: 'recycle',
      amount: parseFloat(recycleReward).toFixed(2),
      balance: parseFloat(user.points).toFixed(2),
      ...
      metadata: {
        ...
        dollName: doll.name,
        remainingDays: doll.remainingDays,
        recycleReward: parseFloat(recycleReward).toFixed(2)
      }
    }).save();
    ...
    res.json({
      ...
      message: '回收成功',
      data: {
        recyclePoints: parseFloat(recycleReward).toFixed(2),
        experience: parseFloat(recycleExperience).toFixed(2),
