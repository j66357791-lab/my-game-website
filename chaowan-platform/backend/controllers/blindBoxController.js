// backend/controllers/blindBoxController.js - 完整版本
...
exports.getActivityData = async (req, res) => {
  try {
    ...
    if (!activityData) {
      ...
      console.log('✅ 创建新盲盒活动数据:', userId);
    }
    
    console.log('✅ 获取盲盒活动数据成功，背包字符:', activityData.collectedChars.length, '个');
...
exports.singleDraw = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId);
    if (user.points < 10) {
      return res.status(400).json({ success: false, message: '积分不足，需要10积分' });
    }
...
exports.tenDraw = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId);
    if (user.points < 95) {
      return res.status(400).json({ success: false, message: '积分不足，需要95积分' });
    }
...
// 🔧 关键修复：兑换奖励（检查炼化工厂中的字符）
exports.exchangeReward = async (req, res) => {
  try {
    const { chars } = req.body;
    // 🔧 修复：使用 _id 而不是 id
    const userId = req.user._id;
    console.log('🔥 盲盒兑换奖励 - 用户ID:', userId, '字符:', chars);
    ...
    const factory = await RefiningFactory.findOne({ userId });
    
    // 🔧 关键修复：计算实际可用的字符（背包字符 - 炼化工厂中的字符）
    const availableChars = [...(userActivity.collectedChars || [])];
    
    // 如果炼化工厂中有字符，需要从可用字符中排除
    if (factory && factory.inputChars && factory.inputChars.length > 0) {
      console.log(`🔍 炼化工厂中有${factory.inputChars.length}个字符，需要排除`);
      factory.inputChars.forEach(factoryChar => {
        const index = availableChars.indexOf(factoryChar);
        if (index > -1) {
    ...
    const hasEnoughChars = chars.every(char => {
      ...
      const countInAvailable = availableChars.filter(c => c === char).length;
      return countInRequest <= countInAvailable;
    });
    ...
    if (!hasEnoughChars) {
      ...
      for (const [char, needed] of Object.entries(charCounts)) {
        const available = availableChars.filter(c => c === char).length;
        if (available < needed) {
          insufficientChars.push(`${char}(需要${needed}个，可用${available}个)`);
        }
      }
      
      return res.status(400).json({ 
        success: false, 
        message: `字符不足: ${insufficientChars.join(', ')}。注意：炼化工厂中的字符无法用于兑换。` 
      });
    }
    ...
    console.log(`📊 兑换后背包剩余字符: ${userActivity.collectedChars.length}个`);
    ...
    res.json({ 
      ...
      data: { 
        ...
        charsUsed: chars,
        // 🔧 新增：返回更新后的背包数据
        remainingChars: userActivity.collectedChars,
        remainingCharsCount: userActivity.collectedChars.length
      } 
    });
  } catch (error) {
...
exports.getExchangeHistory = async (req, res) => {
  try {
    ...
    console.log(`📋 获取兑换历史 - 用户ID: ${userId}, 记录数: ${rewards.length}`);
