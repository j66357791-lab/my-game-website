...
import RefiningFactoryPage from './pages/RefiningFactoryPage'; // 🔧 新增：导入炼化工厂页面
...
function AppContent() {
  ...
  const handleLogin = async (userData, token) => {
    ...
    try {
      await login({ email: userData.email, password: '' }); // 这里需要调整
              ...
              <Routes>
                ...
                <Route
                  ...
                  element={
                    <RefiningFactoryPage 
                      user={user}
                      onUpdateUser={handleUpdateUser} // 🔧 修复：使用正确的函数
                      globalPoints={points}
                      syncUserData={syncUserData}
                    /> 
                  }
