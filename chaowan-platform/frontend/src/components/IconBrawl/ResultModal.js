...
const ResultModal = ({ show, result, myBets, isWin, onClose, onPlayAgain }) => {
  ...
  const totalBet = Object.values(myBets).reduce((sum, amount) => sum + amount, 0);
      ...
      <div className="result-modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="result-header">
          ...
          <div className="result-summary">
            ...
            <div className="result-details">
              ...
              <span>组合: {result?.combination?.name}</span>
              <span>倍率: {result?.combination?.multiplier}x</span>
            </div>
          </div>
        </div>
        ...
        <div className="result-body">
          ...
          <div className="result-info">
            <p>组合类型: {result?.combination?.name}</p>
