  ...
  const getIconStats = () => {
    ...
    if (gameHistory && Array.isArray(gameHistory)) {
      gameHistory.forEach(session => {
        if (session && session.icons && Array.isArray(session.icons)) {
                  ...
                  gameHistory.map((session, index) => (
                    <div key={index} className="result-item">
                      <span className="session-id">
                        {/* 🔧 使用真实的局号 */}
                        第{String(session.sessionNumber || index + 1).padStart(6, '0')}期
                      </span>
                      <span className="result-icons">
                        {session.icons && session.icons.map(icon => 
                          (icons && icons[icon]?.symbol) || icon
                        ).join(' ')}
                      </span>
                      <span className="result-combination">
                        {session.combination?.name || '未知'}
                      </span>
                      <span className="result-time">
                        {session.timestamp ? new Date(session.timestamp).toLocaleTimeString() : ''}
                      </span>
                      <span className={`result-net ${session.result > 0 ? 'win' : 'lose'}`}>
                        {session.result > 0 ? '+' : ''}{session.result || 0}
                      </span>
                    </div>
                  ))
                ) : (
              ...
              <div className="stats-grid">
                {Object.entries(getIconStats()).map(([key, stat]) => (
                  <div key={key} className="stat-item">
                    <span className="stat-icon">{icons && icons[key]?.symbol}</span>
                    <span className="stat-value">{stat.percentage}%</span>
                    <span className="stat-label">{icons && icons[key]?.name}</span>
                  ...
                  <div className="trend-list">
                    {hot.map(([key, stat]) => (
                      <div key={key} className="trend-item hot">
                        <span className="trend-icon">{icons && icons[key]?.symbol}</span>
                        <span className="trend-name">{icons && icons[key]?.name}</span>
