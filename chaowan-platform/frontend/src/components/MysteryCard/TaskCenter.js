// frontend/src/components/MysteryCard/TaskCenter.js - 任务中心组件
...
const TaskCenter = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    // 检查任务完成情况
    setDailyTasks(prev => prev.map(task => {
      const progress = user.todayWins || 0;
                ...
                <div className="task-progress">
                  <div className="progress-bar">
                    <div 
                      ...
                      style={{ width: `${Math.min(100, (user.todayWins || 0) / task.target * 100)}%` }}
                    ></div>
                  </div>
                  <span className="progress-text">
                    {user.todayWins || 0} / {task.target}
                  </span>
                </div>
                <button 
