...
const GameBoard = ({ gameState: externalGameState }) => {
  ...
  useEffect(() => {
    ...
    if (currentGameState.roundNumber !== lastRoundRef.current) {
      ...
      setUserTotalBetThisRound(0); // 重置本局下注
    }
  }, [currentGameState]);
  ...
  useEffect(() => {
    ...
    const listenerId = wsService.addListener((message) => {
      ...
      else if (message.type === 'GAME_HISTORY_DATA') { // 🔧 新增：走势图数据
  ...
  const handleGetTrend = () => { if (wsService) wsService.sendMessage({ type: 'GET_GAME_HISTORY' }); }; // 🔧 新增
    ...
    <div className="game-board">
      <div className="game-header">
        ...
        <div className="game-info">
          ...
          <span className="bet-status-tag">本局下注: {userTotalBetThisRound}</span> {/* 🔧 新增显示 */}
        </div>
      </div>
