...
const GameBoard = ({ gameState: externalGameState }) => {
  ...
  useEffect(() => {
    ...
    const listenerId = wsService.addListener((message) => {
      if (message.type === 'SETTLEMENT') {
        ...
        // 播放音效
        const hasWin = results.results?.some(r => r.result === 'win' || r.result === 'WIN');
        if (hasWin) soundManager.playWin(); else soundManager.playLose();

        // 🔥 核心优化：直接使用后端计算的 totalWinAmount
        // 后端已经帮我们算好了总赢取额，不需要前端再遍历计算，避免逻辑不一致
        const roundWin = results.totalWinAmount || 0;

        console.log(`📊 后端计算本轮赢取: ${roundWin}`);

        if (roundWin > 0) {
          console.log('✅ 调用 updateTodayWins 更新任务进度');
          try {
            updateTodayWins(roundWin);
          } catch (err) {
            console.error('❌ 调用 updateTodayWins 失败:', err);
          }
        } else {
          console.log('ℹ️ 本局未赢取积分，不更新进度');
        }
      }
  ...
  }, [wsService, user, updateUser, setLastSettlement, updateTodayWins]);
