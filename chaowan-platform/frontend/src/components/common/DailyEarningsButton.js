// src/components/common/DailyEarningsButton.js - 修复版
...
const DailyEarningsButton = ({ onEarningsClaimed }) => {
  ...
  const checkClaimStatus = async () => {
    try {
      ...
      if (data.success) {
        ...
        if (todayTransaction) {
          setTodayEarnings(parseFloat(todayTransaction.amount || 0));
        }
      }
    } catch (error) {
  ...
  const handleClaimEarnings = async () => {
    ...
    try {
      ...
      if (data.success) {
        setCanClaim(false);
        setTodayEarnings(parseFloat(data.data.totalEarnings || 0));
        if (onEarningsClaimed) {
          onEarningsClaimed(data.data);
        }
        // 🔧 修复：添加精度处理
        alert(`成功领取今日收益 ${parseFloat(data.data.totalEarnings || 0).toFixed(2)} 积分！`);
      } else {
        alert(data.message);
      }
    } catch (error) {
    ...
    <div className="daily-earnings-button">
      {canClaim ? (
        ...
        <div className="claimed-info">
          <span className="claimed-text">✅ 今日已领取 {parseFloat(todayEarnings || 0).toFixed(2)} 积分</span>
