...
const DailyEarningsButton = ({ onEarningsClaimed }) => {
  ...
  const checkClaimStatus = async () => {
    try {
      ...
      const response = await fetch('https://tianchang.zeabur.app/api/transactions/cash', {
        headers: {
  ...
  const handleClaimEarnings = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://tianchang.zeabur.app/api/dolls/claim-earnings', {
        method: 'POST',
        headers: {
