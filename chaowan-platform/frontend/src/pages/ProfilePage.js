// src/pages/ProfilePage.js - V7.6.0 修复现金明细版 + 收益领取功能
import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { api } from '../config/api';
import { useUser } from '../contexts/UserContext';
import DailyEarningsButton from '../components/common/DailyEarningsButton'; // 🔧 新增导入
...
const ProfilePage = ({ user, onLogout }) => {
  ...
  const handleEarningsClaimed = (data) => {
    ...
    if (data.newPoints !== undefined) {
      ...
      // 这里应该调用父组件的更新函数，但为了简化，我们直接刷新
      window.location.reload(); // 简化处理，实际应该使用状态更新
    }
  };
