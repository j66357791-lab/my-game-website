// frontend/src/pages/ProfilePage.js - V7.6.0 修复现金明细版 + 安全格式化
    ...
    try {
      ...
      if (data.success) {
        ...
        const mockPointsHistory = [
          { id: 1, description: '每日签到奖励', amount: 10, type: 'checkin', createdAt: '2024-01-16' },
          { id: 2, description: '炼化积分 - 完成5个字符', amount: 50, type: 'refining_claim', createdAt: '2024-01-15' },
          { id: 3, description: '盲盒单抽', amount: -10, type: 'blindbox_draw', createdAt: '2024-01-14' },
          { id: 4, description: '盲盒奖励兑换 - 内测红包', amount: 1.58, type: 'blindbox_reward', createdAt: '2024-01-13' }
        ];
                      ...
                      pointsHistory.map((item) => (
                        <div key={item.id} className="history-item">
                          ...
                          <div className={`history-amount ${item.amount > 0 ? 'positive' : 'negative'}`}>
                            {item.amount > 0 ? '+' : '-'}{safeToFixed(item.amount)}
                          </div>
                        </div>
                      ))
                    ) : (
