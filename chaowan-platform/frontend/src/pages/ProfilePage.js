// src/pages/ProfilePage.js - V7.4.1 全局状态融合版
...
const ProfilePage = ({ user, onLogout }) => {
  ...
  // 🔧 V7.4.1 移除本地现金状态，使用全局状态
  // const [cashBalance, setCashBalance] = useState(user.cashBalance || 0);

  // 🔧 V7.4.1 移除本地娃娃获取，使用全局状态
  // const fetchMyDolls = async () => { ... };
    ...
    try {
      ...
      if (savedHistory) {
        ...
        const mockPointsHistory = [
          { id: 1, description: '每日签到奖励', amount: 10, type: 'points', createdAt: '2024-01-16' },
          { id: 2, description: '娃娃产出收益', amount: 5.2, type: 'points', createdAt: '2024-01-15' },
          { id: 3, description: '等级升级奖励', amount: 50, type: 'points', createdAt: '2024-01-14' },
          { id: 4, description: '每日签到奖励', amount: 10, type: 'points', createdAt: '2024-01-13' },
          { id: 5, description: '娃娃产出收益', amount: 3.8, type: 'points', createdAt: '2024-01-12' }
        ];
  ...
  const fetchCashHistory = async () => {
    try {
      ...
      // 🔧 V7.4.1 优先从localStorage获取历史记录
      const savedHistory = localStorage.getItem('cashHistory');
      if (savedHistory) {
        setCashHistory(JSON.parse(savedHistory));
      } else {
        // 备用模拟数据
        const mockCashHistory = [
          { id: 1, description: '积分兑换现金', amount: 100, type: 'cash', status: 'completed', createdAt: '2024-01-15' },
          { id: 2, description: '推荐奖励', amount: 50, type: 'cash', status: 'completed', createdAt: '2024-01-10' },
  ...
  const fetchWithdrawHistory = async () => {
    try {
      ...
      const mockWithdrawHistory = [
        { id: 1, amount: 100, status: 'success', date: '2024-01-15', alipayAccount: '138****1234', processTime: '2024-01-15 14:30' },
        { id: 2, amount: 50, status: 'pending', date: '2024-01-16', alipayAccount: '138****1234', processTime: '处理中...' },
        { id: 3, amount: 30, status: 'rejected', date: '2024-01-14', alipayAccount: '138****1234', reason: '余额不足', processTime: '2024-01-14 16:20' }
      ];
  ...
  const handleWithdraw = (amount) => {
    ...
    const confirmMessage = `确认提现 ¥${amount} 到支付宝 ${alipayAccount}？\n\n提现流程：\n1. 提交申请\n2. 管理员审核\n3. 审批通过后打款`;
            ...
            <div className="wallet-section">
              <div className="points-card">
                ...
                <div className="balance-amount">
                  <span className="amount">{points.toLocaleString()}</span>
                ...
                <div className="wallet-actions">
                  ...
                  <button className="earn-btn" onClick={() => navigate('/mall')}>
                    🛒 赚取积分
                  </button>
                </div>
              </div>
                    ...
                    {loading ? (
                      ...
                      pointsHistory.map((item) => (
                        <div key={item.id} className="history-item">
                          ...
                          <div className="history-amount points">
                            +{item.amount}
                          </div>
                        </div>
                      ))
                    ...
                    {cashHistory.length > 0 ? (
                      cashHistory.map((item) => (
                        <div key={item.id} className="history-item">
                          ...
                          <div className="history-amount cash">
                            +¥{item.amount}
                          </div>
                        </div>
                      ))
              ...
              {filteredDolls.length > 0 ? (
                filteredDolls.map((doll) => (
                  <div key={doll.id} className="doll-card-container">
                    ...
                    >
                      {globalLoading ? '处理中...' : `♻️ 回收 (${Math.floor(0.5 * doll.daysLeft)}积分)`}
                    </button>
                  </div>
                ))
              ) : (
          ...
          <div className="withdraw-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="withdraw-user-info">
              ...
              <div className="user-info-row">
                ...
                <span className="info-value balance">¥{cashBalance}</span>
              </div>
            </div>
              ...
              <div className="amount-grid">
                <div className="amount-row">
                  <button 
                    ...
                    disabled={!isAlipayBound || cashBalance < 10}
                  >
                    ¥10
                  </button>
                  <button 
                    ...
                    disabled={!isAlipayBound || cashBalance < 30}
                  >
                    ¥30
                  </button>
                  <button 
                    ...
                    disabled={!isAlipayBound || cashBalance < 50}
                  >
                    ¥50
                  </button>
                </div>
                <div className="amount-row">
                  <button 
                    ...
                    disabled={!isAlipayBound || cashBalance < 100}
                  >
                    ¥100
                  </button>
                  <button 
                    ...
                    disabled={!isAlipayBound || cashBalance < 300}
                  >
                    ¥300
                  </button>
                  <button 
                    ...
                    disabled={!isAlipayBound || cashBalance < 500}
                  >
                    ¥500
                  </button>
                </div>
                    ...
                    withdrawHistory.map((item) => (
                      <div key={item.id} className="withdraw-item">
                        <div className="withdraw-info">
                          <p className="withdraw-amount">¥{item.amount}</p>
                          <p className="withdraw-account">{item.alipayAccount}</p>
                          <p className="withdraw-date">{item.date}</p>
                          <p className="process-time">处理时间: {item.processTime}</p>
                        </div>
                        <div className={`withdraw-status ${item.status}`}>
                          {item.status === 'success' && '✅ 已成功'}
            ...
            <div className="doll-detail-info">
              ...
              <div className="doll-details">
                ...
                <div className="detail-row">
                  ...
                  <span className="detail-value">+{selectedDoll.output} 积分</span>
                </div>
                <div className="detail-row">
                ...
                <div className="detail-row">
                  ...
                  <span className="detail-value">+{selectedDoll.totalEarned} 积分</span>
                </div>
                <div className="detail-row">
                ...
                <div className="detail-row">
                  ...
                  <span className="detail-value recycle">{Math.floor(0.5 * selectedDoll.daysLeft)} 积分</span>
                </div>
              </div>
            </div>
          </div>
