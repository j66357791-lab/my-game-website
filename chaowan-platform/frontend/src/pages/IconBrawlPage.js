...
export default function IconBrawlPage() {
  ...
  const [totalBets, setTotalBets] = useState({}); // 🔧 现在totalBets完全来自后端
  ...
  const handleGameResult = async (icons) => {
    try {
      ...
      // 🔧 allParticipantBets 现在直接使用 totalBets (来自后端)
      ...
      const newHistoryItem = {
        ...
        totalPlayers: 101, // 这个可以写死，或者通过计算后端返回的非机器人玩家数
