...
const generateAIBets = () => {
  ...
  // 生成100个AI玩家的下注
  for (let i = 0; i < 100; i++) {
    const betCount = Math.floor(Math.random() * 3) + 1; // 每个AI下注1-3个图标
    
    for (let j = 0; j < betCount; j++) {
      const icon = iconKeys[Math.floor(Math.random() * iconKeys.length)];
      const amount = Math.floor(Math.random() * 50) + 1; // 1-50积分
      
      aiBets[icon] = (aiBets[icon] || 0) + amount;
    }
  }
...
export default function IconBrawlPage() {
  ...
  // 🔧 获取本局总下注（AI + 用户已确认 + 用户待确认）
  const getCurrentPot = () => {
    const aiTotal = Object.values(aiBets).reduce((sum, amount) => sum + amount, 0);
    const userConfirmedTotal = Object.values(myBets).reduce((sum, amount) => sum + amount, 0);
    const userPendingTotal = Object.values(pendingBets).reduce((sum, amount) => sum + amount, 0);
    return aiTotal + userConfirmedTotal + userPendingTotal;
  };
  ...
  }, [user?.id, game?.session_id, aiBets]);
  ...
  // 🔧 修复：正确的结算机制（用户只需要中任意一个图标就算胜利）
  const handleGameResult = async (icons) => {
    try {
      const winningIcons = [...new Set(icons)]; // 翻开的图标都是胜利图标
      
      // 🔧 计算所有参与者的下注
      const allParticipantBets = { ...aiBets };
      Object.entries(myBets).forEach(([icon, amount]) => {
        allParticipantBets[icon] = (allParticipantBets[icon] || 0) + amount;
      });
