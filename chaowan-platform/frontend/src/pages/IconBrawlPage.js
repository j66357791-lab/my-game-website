...
export default function IconBrawlPage() {
  ...
  const startNewGame = () => {
    ...
    aiData.forEach(ai => {
      ai.bets.forEach(bet => {
        allBetsData[bet.icon] = (allBetsData[bet.icon] || 0) + bet.amount;
      });
    });
  ...
  useEffect(() => {
    const fetchGame = async () => {
      try {
        ...
        // 合并AI下注和真实用户下注
        const allBetsData = { ...allBets };
        const userBets = {};
        
        if (res.bets && Array.isArray(res.bets)) {
          res.bets.forEach(bet => {
            if (bet.icon_type) {
              allBetsData[bet.icon_type] = (allBetsData[bet.icon_type] || 0) + (bet.bet_amount || 0);
  ...
  }, [user?.id, game?.session_id, allBets]);
  ...
  const handleGameResult = async (icons) => {
    try {
      ...
      // 🔧 修复：计算所有参与者的下注（包括AI）
      const allParticipantBets = {};
      
      // 添加AI下注
      aiBets.forEach(ai => {
        ai.bets.forEach(bet => {
          allParticipantBets[bet.icon] = (allParticipantBets[bet.icon] || 0) + bet.amount;
        });
      });
        ...
        <div className="bet-buttons-row">
          {Object.entries(ICONS).map(([key, icon]) => (
            <BetButton
              key={key}
              icon={icon}
              totalBet={(allBets && allBets[key]) || 0}
