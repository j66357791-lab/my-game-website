...
export default function IconBrawlPage() {
  ...
  // 🔧 关键修改：从 useUserData 获取 refreshUserData 函数来刷新用户数据
  const { user, points: realPoints, updatePoints, refreshUserData } = useUserData();
  ...
  // 🔧 核心重构：结算逻辑 - 只负责UI反馈和触发数据刷新
  const handleGameResult = async (icons) => {
    try {
      const winningIcons = [...new Set(icons)]; // 获取所有胜利图标
      
      // 1. 计算用户的净收益（用于UI显示）
      let userNetResult = 0;
      const totalUserBet = Object.values(myBets).reduce((sum, amount) => sum + amount, 0);
      
      if (totalUserBet > 0) {
        const userWinningBet = Object.entries(myBets)
          .filter(([icon]) => winningIcons.includes(icon))
          .reduce((sum, [, amount]) => sum + amount, 0);

        if (userWinningBet > 0) {
          // 用户有下注中的图标，计算奖励
          const allWinningBets = Object.entries(totalBets)
            .filter(([icon]) => winningIcons.includes(icon))
            .reduce((sum, [, amount]) => sum + amount, 0);
          
          const allLosingBets = Object.values(totalBets).reduce((sum, amount) => sum + amount, 0) - allWinningBets;
          
          const userReward = (userWinningBet / allWinningBets) * allLosingBets;
          userNetResult = Math.floor(userReward);
        } else {
          // 用户下注的图标全输了
          userNetResult = -totalUserBet;
        }
      }
