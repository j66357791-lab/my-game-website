...
const LoginPage = ({ onLogin }) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (typeof onLogin === 'function') {
          ...
          console.error('❌ onLogin不是函数:', typeof onLogin);
        }
  ...
  const testConnection = async () => {
    try {
      ...
      // 测试CORS
      const response = await fetch('https://tianchuang.onrender.com/api/test-cors');
      const data = await response.json();
      console.log('✅ CORS测试成功:', data);
      
      // 测试登录API连通性
      const loginResponse = await fetch('https://tianchuang.onrender.com/api/auth/login', {
        method: 'POST',
        headers: {
      ...
      console.log('✅ 网络连接正常:', loginResponse.status);
      alert('网络连接正常！');
      return true;
    } catch (error) {
