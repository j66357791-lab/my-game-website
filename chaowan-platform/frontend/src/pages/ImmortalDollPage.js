...
const ImmortalDollPage = ({ syncUserData }) => {
  ...
  const [battleResult, setBattleResult] = useState(null); // 胜利后的掉落
  ...
  const handleChallenge = async (stageId) => {
    setIsOperating(true);
    try {
      setBattleAnimVisible(true); // 先打开动画遮罩
      ...
      if (res.data.battleResult) {
        ...
        const interval = setInterval(() => {
          if (step >= logs.length) {
            ...
            // 战斗动画结束，显示结果
            if (res.success) {
              setBattleResult(res.data); // 保存掉落信息
            }
          } else {
  ...
  const maxSpirit = (nextLevelConfig ? nextLevelConfig.cost : 100000); // 简单的进度条参考值
  const spiritPercent = Math.min(100, (doll.spiritualEnergy / 80000) * 100); // 以凡人十级为满球参考
        ...
        {activeTab === 'home' && (
          <div className="panel home-panel">
            ...
            <p style={{color:'#7f8c8d', fontSize:'12px', textAlign:'center', marginTop:'10px'}}>
                当前境界{doll.realm}最大等级: {config && config.getPoolMax && config.getPoolMax(doll.realm) || '?'}
            </p>
          </div>
        )}
