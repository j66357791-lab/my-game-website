...
const ImmortalDollPage = ({ syncUserData }) => {
  ...
  const [selectedEquipId, setSelectedEquipId] = useState(null); // 当前选中操作的装备ID (不论是在角色身上还是在背包里)
  ...
  const handleEquip = async () => {
    ...
    // 如果当前就在身上，则不操作
    if (doll.equipmentSlots[equip.slot] && doll.equipmentSlots[equip.slot]._id === selectedEquipId) return;
    
    setIsOperating(true);
    try {
      const res = await api.post('/immortal/equipment/equip', { equipId });
      if (res.success) {
        setDoll(res.data.doll);
        fetchInventory(); // 更新背包
      }
    ...
    if (doll.equipmentSlots[equip.slot] && doll.equipmentSlots[equip.slot]._id === selectedEquipId) {
      ...
      try {
        ...
        if (res.success) {
          setDoll(res.data.doll);
          fetchInventory();
          setSelectedEquipId(null); // 卸下后取消选中
        }
      } catch (e) { alert('操作失败'); }
      finally { setIsOperating(false); }
    } else {
  ...
  const handleRefine = async () => {
    ...
    // 自动寻找背包里第一个同部位装备作为材料（简化UI，实际可用选择弹窗）
    const material = inventory.find(i => i.slot === equip.slot && i._id !== selectedEquipId);
    if (!material) return alert(`背包中缺少${equip.slot}作为强化材料！`);
    
    // 检查强化石
    const user = await api.get('/auth/me'); // 这里需有获取当前用户接口，或前端存储
    // 简化：假设有强化石，或者后端会返回不足
    if (!window.confirm(`消耗1个${equip.slot}和1个强化石进行强化？成功率: ${Math.max(0.1, (0.6 - equip.level * 0.03)*100).toFixed(0)}%`)) return;
  ...
  const handleStarUp = async () => {
    if (!selectedEquipId) return;
    const equip = getAllEquips().find(e => e._id === selectedEquipId);
    if (equip.star >= 3) return alert('已达到最高星级');
  ...
  const handleDecompose = async () => {
    ...
    if (!window.confirm(`确认拆解此装备？将回收星源币。`)) return;
    setIsOperating(true);
    try {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="slot-content">
                      {equippedId ? (
                        <div className="equipped-star">
                          ⭐{(doll.equipmentSlots[slot].star || 0)}
                        </div>
                      ) : (
