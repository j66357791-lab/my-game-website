...
const ImmortalDollPage = ({ syncUserData }) => {
  ...
  const [battleResult, setBattleResult] = useState(null); // ✅ 新增战斗结果状态
  ...
  const handleChallenge = async () => {
    if (battleResult) return; // 防止重复点击

    setIsOperating(true);
    try {
      const res = await api.post('/immortal/dungeon/challenge');
      
      // 无论成功失败，都更新doll和user数据
      if (res.success) {
        setDoll(res.data.doll);
        if (syncUserData) syncUserData(); // 同步星源币消耗
      } else {
        // 业务失败（战败），但接口可能返回200（如果我们要展示失败结算）
        if (res.data.doll) setDoll(res.data.doll);
        if (res.data.userStarcoin && syncUserData) syncUserData();
      }
      
      setBattleResult(res); // 设置结果触发弹窗
            ...
            {battleResult.success ? (
                <div className="rewards-area">
                    ...
                    <div className="reward-item">
                        <span>灵气:</span>
                        <span className="val">+{battleResult.data.rewards.spirit}</span>
                    </div>
                    ...
                    {battleResult.data.rewards.equipments.length > 0 && (
                        <div className="reward-section">
                            ...
                            {battleResult.data.rewards.equipments.map((eq, idx) => (
                                <div key={idx} className="equip-tag">{eq.slot} Lv.{eq.level}</div>
                            ))}
                        </div>
                    )}
