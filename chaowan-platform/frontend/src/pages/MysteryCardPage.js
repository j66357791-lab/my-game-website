...
const MysteryCardPage = ({ user, onUpdateUser, globalPoints, globalCash, globalDolls, syncUserData }) => {
  ...
  // 🔧 新增：跟踪初始数据是否已全部收到
  const [initialDataReceived, setInitialDataReceived] = useState({
    gameState: false,
    history: false
  });

  // 🔧 第一个 useEffect：处理 WebSocket 连接
  useEffect(() => {
    if (!user || !token) {
      navigate('/login');
      return;
    ...
    const handleInitialLoad = (message) => {
      ...
      // 🔧 修复：分别标记 gameState 和 history 是否收到
      if (message.type === 'GAME_STATE') {
        setInitialDataReceived(prev => ({ ...prev, gameState: true }));
      }
      
      if (message.type === 'HISTORY') {
        setInitialDataReceived(prev => ({ ...prev, history: true }));
      }
    };
  ...
  }, [user, token, navigate]); // 🔧 依赖项保持不变

  // 🔧 第二个 useEffect：监听 initialDataReceived 变化，关闭加载弹窗
  useEffect(() => {
    if (initialDataReceived.gameState && initialDataReceived.history) {
      setIsLoading(false);
    }
  }, [initialDataReceived.gameState, initialDataReceived.history]);
