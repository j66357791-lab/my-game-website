...
const RefiningFactoryPage = ({ user, onUpdateUser, globalPoints, syncUserData }) => {
  ...
  const loadFactoryData = useCallback(async () => {
    try {
      const data = await api.get('/refiningFactory/data');
  ...
  const handleInputChars = async () => {
    ...
    try {
      const result = await api.post('/refiningFactory/input', { chars: inputChars });
      setSuccess('投入成功');
      setInputChars([]);
      loadFactoryData();
  ...
  const handleWithdrawChars = async () => {
    try {
      const result = await api.post('/refiningFactory/withdraw');
  ...
  const handleClaimPoints = async () => {
    try {
      const result = await api.post('/refiningFactory/claim');
