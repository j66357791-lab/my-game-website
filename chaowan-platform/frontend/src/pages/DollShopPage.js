// src/pages/DollShopPage.js - 修复积分精度和错误处理
...
const DollShopPage = () => {
  ...
  const handleSinglePurchase = async (doll) => {
    ...
    try {
      ...
      if (data.success) {
        // 🔧 安全处理响应数据
        const newPoints = parseFloat(data.data?.newPoints) || userPoints;
        const newExperience = parseFloat(data.data?.newExperience) || parseFloat(user.experience) || 0;
        ...
        if (experienceGained > 0) {
          successMessage += ` 获得 ${experienceGained} 经验`;
        }
    ...
    <div className="doll-shop-page">
      <div className="shop-header">
        ...
        <div className="user-info">
          {/* 🔧 修复积分显示精度 */}
          <span>积分: {user ? parseFloat(user.points || 0).toFixed(2) : '0.00'}</span>
          <span>经验: {user ? parseFloat(user.experience || 0).toFixed(2) : '0.00'}</span>
        </div>
      </div>
        ...
        {shopDolls.map(doll => (
          <div key={doll.level} className="doll-card">
            ...
            <div className="doll-info">
              ...
              <p>稀有度: {doll.rarity}</p>
              {/* 🔧 修复价格显示精度 */}
              <p>价格: {parseFloat(doll.purchasePrice || 0).toFixed(2)} 积分</p>
              <p>每日产出: {parseFloat(doll.productionPerDay || 0).toFixed(2)} 积分</p>
      ...
      {batchMode && (
        <div className="batch-summary">
          <div className="summary-info">
            ...
            <span>费用: {parseFloat(calculateTotalCost() || 0).toFixed(2)} 积分</span>
          </div>
          <button 
