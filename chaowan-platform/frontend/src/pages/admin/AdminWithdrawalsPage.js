...
const AdminWithdrawalsPage = ({ user, onUpdateUser }) => {
  ...
  const fetchWithdrawals = async (page = 1) => {
    try {
      ...
      let url = `https://tianchang.zeabur.app/api/admin/withdrawals?status=${filter}&page=${page}&limit=10`;
  ...
  const handleProcessWithdrawal = async (withdrawalId, action, remark = '') => {
    ...
    try {
      ...
      const response = await fetch(`https://tianchang.zeabur.app/api/admin/withdrawals/${withdrawalId}/process`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleBatchProcess = async (action) => {
    ...
    try {
      ...
      const response = await fetch('https://tianchang.zeabur.app/api/admin/withdrawals/batch-process', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
