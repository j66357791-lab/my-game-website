...
const AdminUsersPage = ({ user, onUpdateUser }) => {
  ...
  const handleUpdateUser = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://tianchuang.onrender.com/api/admin/users/${selectedUser._id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleChangePassword = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://tianchuang.onrender.com/api/admin/users/${selectedUser._id}/password`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleAdjustCash = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://tianchuang.onrender.com/api/admin/cash/adjust', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleToggleUserStatus = async (userId, username, currentStatus) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://tianchuang.onrender.com/api/admin/users/${userId}/toggle-status`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const openEditModal = (userItem) => {
    ...
    setEditForm({
      ...
      cashBalance: userItem.cashBalance || 0, // 🔧 新增
      role: userItem.role,
                ...
                <tbody>
                  {users.map((userItem) => (
                    <tr key={userItem._id} className={userItem.disabled ? 'disabled-user' : ''}>
                      ...
                      <td>¥{(userItem.cashBalance || 0).toFixed(2)}</td> {/* 🔧 新增 */}
                      <td>
