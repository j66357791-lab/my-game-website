    ...
    return (
        <div className="admin-page-container">
            ...
            <table className="admin-table">
                <thead><tr><th>用户名</th><th>邮箱</th><th>等级</th><th>积分</th><th>角色</th><th>操作</th></tr></thead>
                <tbody>{users.map(user => (<tr key={user._id}>
                    <td>{user.username}</td><td>{user.email}</td><td>{user.level}</td><td>{user.points}</td><td>{user.role}</td>
                    <td><button onClick={() => handleAdjustPoints(user._id, user.points)}>调整积分</button><button onClick={() => handleDeleteUser(user._id, user.username)}>删除</button></td>
                </tr>))}</tbody>
            </table>
            <div className="pagination-controls">
                <button onClick={() => fetchUsers(pagination.page - 1)} disabled={pagination.page <= 1}>上一页</button>
                <span>第 {pagination.page} 页 / 共 {pagination.pages} 页</span>
                <button onClick={() => fetchUsers(pagination.page + 1)} disabled={pagination.page >= pagination.pages}>下一页</button>
            </div>
        </div>
    );
};
