// frontend/src/pages/AdminDashboardPage.js - 终极修复版
...
const AdminDashboardPage = ({ user }) => {
  ...
  const [categories, setCategories] = useState([]); // ✅ 恢复分类状态
  ...
  const fetchCategories = async () => {
    try {
      const res = await api.get('/categories'); // 调用新增的接口
      if (res.success) {
        setCategories(res.data || []);
      }
    } catch (error) {
      console.error('获取分类失败(可能暂无分类接口)', error);
    }
  };
  ...
  const fetchAdminData = async () => {
    ...
    try {
      ...
      console.log('🔍 [前端] 正在请求订单列表，筛选参数:', params);
