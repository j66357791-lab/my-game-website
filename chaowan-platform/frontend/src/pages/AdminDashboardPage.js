// frontend/src/pages/AdminDashboardPage.js - 完整功能版 (含退款)
...
const AdminDashboardPage = ({ user }) => {
  ...
  const confirmRefund = async (orderId) => {
    ...
    try {
      console.log('正在发起退款请求，订单ID:', orderId);
      const res = await api.put(`/admin/shop/orders/${orderId}/refund`);
      
      if (res.success) {
        alert('✅ 退款成功！款项已退回用户账户，库存已回退。');
        fetchAdminData(); // 刷新列表
      } else {
        alert('❌ 退款失败: ' + (res.message || '未知错误'));
      }
    } catch (error) {
      console.error('退款请求错误:', error);
      alert('❌ 退款失败: ' + (error.response?.data?.message || '网络错误'));
    }
  };
                  ...
                  <div key={order._id} className="list-item order-item" style={{ flexDirection: 'column', alignItems: 'flex-start', padding: '16px', background: 'white' }}>
                    ...
                    <div style={{ width: '100%', display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '10px' }}>
                      ...
                      {order.status === 'paid' && (
                        <>
                          <button 
                            onClick={() => confirmRefund(order._id)} // ✅ 直接调用退款
