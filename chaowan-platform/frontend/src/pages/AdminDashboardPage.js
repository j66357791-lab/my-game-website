// frontend/src/pages/AdminDashboardPage.js - 新增退款功能版
...
const AdminDashboardPage = ({ user }) => {
  ...
  // ✅ 退款弹窗状态
  const [refundModal, setRefundModal] = useState({ show: false, orderId: '' });
  ...
  const openRefundModal = (orderId) => {
    setRefundModal({ show: true, orderId });
  };

  const confirmRefund = async () => {
    if (!window.confirm('⚠️ 确定要退回该订单吗？\n\n1. 积分和现金将退回用户账户\n2. 商品库存将自动回退\n3. 订单将变为已取消状态')) {
      return;
    }

    try {
      await api.put(`/admin/shop/orders/${refundModal.orderId}/refund`);
      alert('✅ 退款成功！款项已退回用户账户，库存已回退。');
      setRefundModal({ ...refundModal, show: false });
      fetchAdminData();
    } catch (error) {
      alert('❌ 退款失败: ' + (error.response?.data?.message || ''));
    }
  };
                  ...
                  <div key={order._id} className="list-item order-item" style={{ flexDirection: 'column', alignItems: 'flex-start', padding: '16px', background: 'white' }}>
                    ...
                    <div style={{ width: '100%', borderBottom: '1px solid #eee', paddingBottom: '10px', marginBottom: '10px' }}>
                      ...
                      <div style={{ color: '#666', fontSize: '14px', marginBottom: '5px' }}>
                        状态: <span style={{ fontWeight: 'bold', color: order.status === 'paid' ? '#faad14' : order.status === 'shipped' ? '#1890ff' : '#52c41a' }}>{getStatusText(order.status)}</span> 
