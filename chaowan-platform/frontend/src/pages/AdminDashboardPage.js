// frontend/src/pages/AdminDashboardPage.js - 完整优化版
            ...
            <div className="section">
              ...
              <div className="order-toolbar">
                <div className="filter-tabs">
                  ...
                  <button className={orderFilter === 'paid' ? 'active' : ''} onClick={() => setOrderFilter('paid')}>待发货</button>
                  <button className={orderFilter === 'shipped' ? 'active' : ''} onClick={() => setOrderFilter('shipped')}>已发货</button>
                  <button className={orderFilter === 'completed' ? 'active' : ''} onClick={() => setOrderFilter('completed')}>已完结</button>
                </div>
                <input 
                  type="text" 
                ...
                />
                <button className="btn-search" onClick={fetchAdminData}>搜索</button>
              </div>
                ...
                {orders.map(order => (
                  <div key={order._id} className="list-item order-item">
                    <div className="info">
                      ...
                      <div className="meta">
                        <span className={`status-tag ${order.status}`}>{getStatusText(order.status)}</span> | 
                      ...
                      {order.status === 'shipped' && order.shipping && (
                        <div className="logistics-info">
                          ...
                          <span>发货方式: {order.shipping.method === 'express' ? '快递' : '无需快递'}</span>
                          {order.shipping.method === 'express' && (
                            <span className="tracking-no">单号: {order.shipping.trackingNumber}</span>
                          )}
                        </div>
                      )}
                    </div>
