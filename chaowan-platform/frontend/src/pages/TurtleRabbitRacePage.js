// TurtleRabbitRacePage.js - 集成音效版本
...
const TurtleRabbitRacePage = () => {
  ...
  // 重置音效触发
  const resetSoundEffect = (effect) => {
    setSoundEffects(prev => ({ ...prev, [effect]: false }));
  };
  ...
  const startGame = () => {
    ...
    // 播放投注音效
    setSoundEffects(prev => ({ ...prev, playBet: true }));
    setTimeout(() => resetSoundEffect('playBet'), 100);
  ...
  const handleRaceComplete = (result) => {
    ...
    if (result.result === 'win') {
      setSoundEffects(prev => ({ ...prev, playWin: true }));
      setTimeout(() => resetSoundEffect('playWin'), 2000);
    } else {
      setSoundEffects(prev => ({ ...prev, playLose: true }));
      setTimeout(() => resetSoundEffect('playLose'), 1000);
    }
  ...
  // 通用点击音效
  const handleClick = () => {
    setSoundEffects(prev => ({ ...prev, playClick: true }));
    setTimeout(() => resetSoundEffect('playClick'), 100);
  };
    ...
    <div className="race-page">
      ...
      <div className="betting-section">
        ...
        <div className="action-buttons">
          ...
          <button onClick={() => {
            alert('游戏规则：选择乌龟或兔子，投入积分或星源币，猜中获得1.9倍奖励！');
            handleClick();
          }}>
            📖 游戏规则
          </button>
      ...
      {showRaceModal && (
        <RaceModal
          ...
          onCountdown={() => setSoundEffects(prev => ({ ...prev, playCountdown: true }))}
        />
      )}
