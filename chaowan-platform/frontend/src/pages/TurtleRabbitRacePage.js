// frontend/src/pages/TurtleRabbitRacePage.js - 修改版本
import React, { useState, useEffect, useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
import { useUser } from '../contexts/UserContext';
import { startRace, getRecentRaces } from '../services/raceService';
...
const TurtleRabbitRacePage = () => {
  ...
  useEffect(() => {
    const fetchRecentRaces = async () => {
      try {
        const response = await getRecentRaces();
        if (response.success) {
          setRaceHistory(response.data);
        }
      } catch (error) {
        console.error('获取最近赛跑结果失败:', error);
      }
    };
