          ...
          <div className="modal-content">
            ...
            <div className="settlement-info">
              {raceResult && (
                <>
                  <p>
                    {raceResult.winner === betResult.betType ? '恭喜您，您猜中了这把' : '别灰心，再来一次，很抱歉您这次损失了'}
                    <strong>{raceResult.winner === 'turtle' ? '乌龟' : '兔子'}</strong>
                    {raceResult.winner === betResult.betType ? '胜利，恭喜您获得' : ''}
                    <strong>{raceResult.winner === betResult.betType ? Math.floor(raceResult.betAmount * 1.9) : raceResult.betAmount}</strong>
                    {raceResult.winner === betResult.betType ? '积分' : '积分'}
                  </p>
                </>
              )}
            </div>
