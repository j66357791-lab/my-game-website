// src/pages/HomePage.js - V7.5.0 修复版 + 收益领取功能
...
const HomePage = ({ user, onUpdateUser }) => {
  ...
  const todayTotalOutput = useMemo(() => {
    return dolls
      ...
      .reduce((sum, doll) => sum + (doll.output || 0), 0);
  }, [dolls]);
  ...
  const refreshUserData = useCallback(async () => {
    ...
    try {
      ...
      // 🔧 修复：直接获取用户信息，不使用api.getUser（可能有问题）
            ...
            {dolls.slice(0, 4).map((doll) => (
              <div key={doll.id} className="doll-item">
                ...
                <div className="doll-info">
                  ...
                  <p className="doll-output">+{doll.output}/天</p>
      ...
      <div className="points-status-card">
        ...
        <div className="points-main">
          <div className="current-points">
            <span className="points-number">{parseFloat(points || 0).toFixed(2).toLocaleString()}</span>
            <span className="points-label">当前积分</span>
          </div>
          <div className="today-output">
            <span className="output-number">+{todayTotalOutput.toFixed(2)}</span>
        ...
        <div className="points-progress">
          ...
          <span className="progress-text">距离下一级还需 {50 - user.experience} 经验</span>
        </div>
      </div>
          ...
          {featuredDolls.map((doll) => (
            <div key={doll.id} className="featured-doll">
              ...
              <div className="featured-info">
                ...
                <p className="featured-output">产出: +{doll.output}/天</p>
