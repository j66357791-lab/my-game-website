// src/pages/DollCenterPage.js - 修正API路径
...
const DollCenterPage = () => {
  ...
  const loadVipData = useCallback(async () => {
    try {
      const timestamp = Date.now();
      const url = `https://tianchuang.onrender.com/api/vipCard/status?t=${timestamp}`;
  ...
  const handleVipPurchase = async (cardType) => {
    ...
    try {
      ...
      const response = await fetch('https://tianchuang.onrender.com/api/vipCard/purchase', {
        method: 'POST',
        headers: getBaseHeaders(),
        body: JSON.stringify({
          type: cardType  // 后端期望的参数名是 type
        })
      });
  ...
  const handleClaimVipReward = async () => {
    ...
    try {
      ...
      const response = await fetch('https://tianchuang.onrender.com/api/vipCard/claim-daily-starcoin', {
        method: 'POST',
        headers: getBaseHeaders()
      });
  ...
  const handleGacha = async (type) => {
    ...
    try {
      ...
      // 使用正确的blindBox API
      const endpoint = type === 'single' ? '/api/blindBox/single-draw' : '/api/blindBox/ten-draw';
      
      const response = await fetch(`https://tianchuang.onrender.com${endpoint}`, {
        method: 'POST',
        headers: getBaseHeaders()
      });
      ...
      if (result.success) {
        if (type === 'single') {
          ...
          alert(`抽取成功！获得${prize.name || '神秘奖品'}`);
        } else {
          const prizes = result.data.prizes || [];
          const prizeNames = prizes.map(p => p.name || '神秘奖品').join(', ');
          alert(`抽取成功！获得: ${prizeNames}`);
        }
    ...
    <div className="gacha-page">
      ...
      <div className="gacha-machines">
        <div className="gacha-machine single">
          ...
          <div className="machine-desc">随机获得奖品</div>
          <button 
        ...
        <div className="gacha-machine multi">
          ...
          <div className="machine-desc">一次抽取10个奖品</div>
          <button 
