// src/pages/DollCenterPage.js - 完整融合修仙版 (无删减)
import React, { useState, useEffect, useMemo, useCallback } from 'react';
import { useNavigate, useSearchParams } from 'react-router-dom';
import { useUser } from '../contexts/UserContext';
import './DollCenterPage.css';

// 引入修仙组件 (请确保这些组件存在于 frontend/src/components/Cultivation/ 目录下)
import CultivationIntro from '../components/Cultivation/CultivationIntro';
import CultivationMain from '../components/Cultivation/CultivationMain';
...
const DollCenterPage = () => {
  ...
  // 修仙相关状态 (新增)
  const [cultivationProfileExists, setCultivationProfileExists] = useState(false);
  ...
  // 3. 修仙功能：检查修仙档案状态
  const checkCultivationStatus = useCallback(async () => {
    try {
      const res = await fetch('https://tianchang.zeabur.app/api/cultivation/data', {
        headers: getBaseHeaders()
      });
      const result = await res.json();
      if (result.success) {
        if (result.exists) {
          setCultivationProfileExists(true);
        } else {
          setCultivationProfileExists(false);
        }
      }
    } catch (e) {
      console.log('修仙系统检查失败');
    }
  }, []);
  ...
  const forceRefreshUserData = useCallback(async () => {
    try {
      ...
      const url = `https://tianchang.zeabur.app/api/auth/user?t=${timestamp}`;
  ...
  const loadVipData = useCallback(async () => {
    try {
      const timestamp = Date.now();
      const url = `https://tianchang.zeabur.app/api/vip-cards/status?t=${timestamp}`;
  ...
  const loadBossData = useCallback(async () => {
    try {
      const timestamp = Date.now();
      const url = `https://tianchang.zeabur.app/api/boss/status?t=${timestamp}`;
  ...
  useEffect(() => {
    ...
    // 新增：如果切换到修仙Tab，检查修仙状态
    if (activeTab === 'cultivation') {
      checkCultivationStatus();
    }
  }, [activeTab, forceRefreshUserData, loadBossData, loadVipData, loadSynthesisData, checkCultivationStatus]);
  ...
  const handleVipPurchase = async (cardType) => {
    ...
    try {
      ...
      const response = await fetch('https://tianchang.zeabur.app/api/vip-cards/purchase', {
        method: 'POST',
        headers: getBaseHeaders(),
        body: JSON.stringify({
          type: cardType
  ...
  const handleClaimVipReward = async () => {
    ...
    try {
      ...
      const response = await fetch('https://tianchang.zeabur.app/api/vip-cards/claim-daily-starcoin', {
        method: 'POST',
        headers: getBaseHeaders()
      });
  ...
  const handleLuckyDraw = async (type) => {
    ...
    try {
      ...
      const response = await fetch('https://tianchang.zeabur.app/api/dolls/lucky-draw', {
        method: 'POST',
        headers: getBaseHeaders(),
        body: JSON.stringify({ 
  ...
  const handleChallengeBoss = async () => {
    ...
    try {
      ...
      const response = await fetch('https://tianchang.zeabur.app/api/boss/attack', {
        method: 'POST',
        headers: getBaseHeaders(),
        body: JSON.stringify({
  ...
  const handleRecycleDoll = async (dollId) => {
    try {
      ...
      const response = await fetch(`https://tianchang.zeabur.app/api/dolls/recycle/${dollId}`, {
        method: 'DELETE',
        headers: getBaseHeaders()
      });
  ...
  const handleSynthesize = async () => {
    ...
    try {
      ...
      const response = await fetch('https://tianchang.zeabur.app/api/dolls/synthesize', {
        method: 'POST',
        headers: getBaseHeaders(),
        body: JSON.stringify({
          dollId: baseDoll._id,
  ...
  const handleDeployDoll = async (dollId) => {
    try {
      ...
      const response = await fetch('https://tianchang.zeabur.app/api/dolls/deploy', {
        method: 'POST',
        headers: getBaseHeaders(),
  ...
  const handleRecallDoll = async (dollId) => {
    try {
      ...
      const response = await fetch('https://tianchang.zeabur.app/api/dolls/recall', {
        method: 'POST',
        headers: getBaseHeaders(),
        body: JSON.stringify({ dollId })
      });
  ...
  const handleClaimDailyEarnings = async () => {
    try {
      ...
      const response = await fetch('https://tianchang.zeabur.app/api/dolls/claim-daily-earnings', {
        method: 'POST',
        headers: getBaseHeaders()
      });
  ...
  const featureModules = [
    ...
    {
      id: 'cultivation', // 新增：修仙之路
      name: '修仙之路',
      icon: '🧘',
      desc: '凡人飞升，突破境界',
      color: '#8e44ad',
      gradient: 'linear-gradient(135deg, #8e44ad, #9b59b6)',
      action: () => handleModuleClick('cultivation')
    }
  ];
  ...
  // 9. 新增：完整修仙页面
  const renderCultivationPage = () => {
    if (!cultivationProfileExists) {
      // 显示性别选择
      return <CultivationIntro onEnterSuccess={() => {
        setCultivationProfileExists(true);
        checkCultivationStatus();
      }} />;
    } else {
      // 显示修仙主页
      return <CultivationMain userData={user} onBack={() => setActiveTab('home')} />;
    }
  };
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'cultivation': // 新增路由
        return renderCultivationPage();
      default:
        return renderHomePage();
    }
  };
