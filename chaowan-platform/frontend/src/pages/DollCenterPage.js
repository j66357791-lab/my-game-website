// src/pages/DollCenterPage.js - 彻底修复CORS
...
const DollCenterPage = () => {
  ...
  const forceRefreshUserData = useCallback(async () => {
    try {
      ...
      // 使用时间戳避免缓存，但不在请求头中设置缓存控制
      const timestamp = Date.now();
      const url = `https://tianchuang.onrender.com/api/user/profile?t=${timestamp}`;
  ...
  const loadVipData = useCallback(async () => {
    try {
      const timestamp = Date.now();
      const url = `https://tianchuang.onrender.com/api/vip/status?t=${timestamp}`;
