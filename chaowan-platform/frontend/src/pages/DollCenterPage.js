// src/pages/DollCenterPage.js - 修复CORS问题
...
const DollCenterPage = () => {
  ...
  const forceRefreshUserData = useCallback(async () => {
    try {
      ...
      // 添加时间戳避免缓存，移除不被允许的Cache-Control头部
      const timestamp = Date.now();
      const response = await fetch(`https://tianchuang.onrender.com/api/user/profile?t=${timestamp}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
          // 移除 Cache-Control: no-cache，因为后端不允许这个头部
        }
      });
  ...
  const loadVipData = useCallback(async () => {
    try {
      const token = localStorage.getItem('token');
      const timestamp = Date.now();
      const response = await fetch(`https://tianchuang.onrender.com/api/vip/status?t=${timestamp}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
          // 移除 Cache-Control: no-cache
        }
      });
