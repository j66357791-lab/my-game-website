// frontend/src/contexts/UserContext.js - 修复版（修正变量名拼写）
import React, { createContext, useState, useEffect, useContext, useCallback } from 'react';
import api from '../config/api'; // 正确导入 api 对象
...
export const UserProvider = ({ children }) => {
  ...
  useEffect(() => {
    const initUser = async () => {
      try {
        ...
        if (savedToken) {
          setToken(savedToken);
          api.setToken(savedToken); // ✅ 修正：使用 api.setToken，而非 Ie.setToken
          await refreshData();
        }
      } catch (err) {
  ...
  const login = async (credentials) => {
    try {
      ...
      if (data.success) {
        ...
        setToken(newToken);
        api.setToken(newToken); // ✅ 修正：使用 api.setToken，而非 Ie.setToken
  ...
  const logout = () => {
    ...
    localStorage.removeItem('token');
    setToken(null);
    api.setToken(null); // ✅ 修正：使用 api.setToken，而非 Ie.setToken
    setError(null);
  };
