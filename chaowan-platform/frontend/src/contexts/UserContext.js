...
export const UserProvider = ({ children }) => {
  ...
  // 🔧 新玩法核心状态
  const [integral, setIntegral] = useState(0); // 使用 integral 替代 points
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (userResponse.success) {
        ...
        setUser(userData);
        setIntegral(userData.integral || 0); // 使用 integral
  ...
  const fetchUserDolls = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await dollService.getDollInventory(token);
      if (response.success) {
  ...
  const updateUser = (newUserData) => {
    ...
    if (newUserData.integral !== undefined) setIntegral(newUserData.integral); // 使用 integral
  ...
  const drawDoll = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const result = await dollService.drawDoll(token);
      if (result.success) {
