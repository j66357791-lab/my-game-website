// frontend/src/components/MysteryCard/GameBoard.js - 最终适配版 (修复同步+弹窗)
...
const GameBoard = ({ gameState }) => {
  ...
  const { user, setUser } = useUser(); // 🔧 关键：必须解构出 setUser 用于更新积分

  // 计算 10% 限制
  const maxBetLimit = user?.points ? Math.floor(user.points * 0.1) : 0;

  // ================= 【关键修复 1】适配新 Context 的监听逻辑 =================
  useEffect(() => {
    // Context 现在将结算数据放在 gameState.results 中
    if (gameState?.currentPhase === 'SETTLEMENT' && gameState?.results) {
      const results = gameState.results;
      setSettlementData(results);
      setShowSettlement(true);

      // 🔧 【关键修复 2】核心：同步更新全局 User 积分
      // 防止前端显示余额与数据库不一致，以及影响下一轮的 10% 限制计算
